...
控制器内部已被环回自检证明完好，片内配置（时钟、复用、`CR`）逐项核对无误，
而 FIFO 始终为空；再加上信号链已按官方图纸逐段确认，**断点就是
`CH9102F pin 21 (TXD) → P2/45 → AC850 45 脚 → MIO49` 这一根网络**。

软件侧没有可做的事，继续改代码不会有进展。

#### 需要动硬件（最小判定步骤）

1. **先看灯**：板子在打印，红色 `D12`（`PS_TX_STATUS`）应常闪；
   在 PC 端敲键盘时黄色 `D13`（`PS_RX_STATUS`）应闪。
   黄灯不闪 → CH9102F 本身没在发；黄灯闪 → 故障在其后。
2. **量 `CH9102F` 第 21 脚**（PC 持续发送时应由 1.8V 跳到 0V 附近）。
   有跳变 → 往后查 P2/45 焊点、连接器、核心板 45 脚；无跳变 → 芯片侧。
3. **旁路验证**：把 `CH9102F` 第 21 脚直接飞线到核心板第 45 脚。
   若串口立刻能收 → 确认是底板走线/连接器问题。
4. 顺带量一下 `R150`（1K 上拉到 1.8V）是否贴装 —— 它不该是故障主因
   （芯片会主动驱动），但缺了会让悬空时电平不定。

板子当前是**交叉组合**（PS 配置取自 `opjtmp.xsa`，PL 比特流取自
`AXI_GPIO_1_SOFT`），没有单一产物能描述正在运行的板子，查硬件前要记住这点。
