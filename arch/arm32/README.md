...
#### 一次**无效**实验（记下来，避免以后当成结论）

曾想用 PS GPIO 的 `DATA_RO` 直接读 MIO49 焊盘电平（它号称与引脚复用功能无关）：
PC 端交替连发 `0x00`（压低）与 `0xFF`（保持高），JTAG 每 250 ms 采样一次。
偏移取自 `gpiops_v3_14/src/xgpiops_hw.h`（`DATA_RO = 0x60`，bank 步进 `0x04`，
MIO[53:32] 属 bank1 → 读 `0xE000A064`，bit16=MIO48、bit17=MIO49）。

结果 240 个采样点里 **bit17 恒为 0** —— 但 **bit16（已知正在被板子驱动的 TX）也恒为 0**。
对照组同时为 0，说明 `DATA_RO` 在引脚被复用给外设后**看不到焊盘**，
本次实验**不成立**，不能据此说"引脚被拉低"。脚本已保留为
`tmp-test/pad_probe.py` + `tmp-test/jtag/sample_mio49.tcl`，
里面特意保留了 TX 对照位的统计 —— 没有对照，这次就会得出错误结论。

#### 结论：断点在片外

把上面的证据合起来，**控制器内部已被环回自检证明完好，片内配置（时钟、
复用、`CR`）逐项核对无误，而 FIFO 始终为空** —— 剩下未被覆盖的环节只有两个：
**MIO49 的焊盘接收器**和**外部连线**。两者都无法从软件侧验证。

所以 M3-7 的"接收方向"**卡在物理链路**上，不是软件问题，继续改代码不会有进展。

#### 待确认（需要动硬件）

板子现状是**交叉组合**：PS 配置取自 `opjtmp.xsa`，PL 比特流取自 `AXI_GPIO_1_SOFT`，
没有任何单一产物能描述正在运行的板子——排查硬件时这点要先记住。

需要上板确认的是 `CH9102F.TXD → 底板 P2/45 → AC850 第 45 脚 → MIO49` 这条链
（发送方向 `MIO48 → P2/27 → CH9102F.RXD` 已实测通畅）：
底板上的跳线/0 Ω/焊接桥是否装好、CH9102F 的 TXD 是否真的接到了 P2/45。
最小判定办法是在 PC 持续发送时，直接量 CH9102F 的 TXD 脚有没有电平跳动。
