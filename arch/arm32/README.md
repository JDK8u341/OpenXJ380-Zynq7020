...
### 11. M4-4：4KB 小页与细粒度映射（**仅宿主验证，尚未接入内核**）
...
| 独立复核 | 拆段后 256 页逐页核对物理地址与属性；属性断言"重建 ≠ 搬运" |
| 全量单测 | 9 failed / 69 passed（与基线一致，无回归）|

**⚠ 尚未做的事**：本模块**还没有接进 `kmain`**，因此**没有在真实页表上验证过**。
上面全部结论来自宿主机上的合成页表。下一步应当是：从 palloc 预留一段
1MB 连续内存，在其上做 `split -> 逐页核对 -> unmap 一页 -> 映射到别的物理页
-> TLB 失效 -> 通过 VA 读回` —— 那是唯一能证明映射真的生效的证据。
