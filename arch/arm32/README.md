...
**正解**：重新导出 XSA 时把 MIO bank 1 的电压设成 **1.8V**，让 Vivado 直接生成正确的
`ps7_init`。这是唯一干净的修法，也能顺带修好 bank 1 上其它外设。

**补偿**：在拿到新 XSA 之前，`tmp-test/zynq/ps7_mio_bank1_18v.tcl` 在 `ps7_init` 之后
把 MIO16-53 的 `[11:9]` 统一改成 1（即每个值 `-0x400`，`0x1600→0x1200`、`0x16E1→0x12E1`）。
`run_kernel_uart.tcl` 已经 source 它并会打印校验结果。

这个机械变换与 vendor 的值互为印证：vendor 正确工程里 `MIO_48` 正是 `0x1200`，
而本 XSA 的 `0x1600` 减 `0x400` 也是 `0x1200`。
