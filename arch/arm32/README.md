...
### M4A-1.3 侦察：SD/arasan 的两半工作面（2026-09-19）

控制器**已经在描述表里**（M3 的成果）：

| 项 | 值 | 出处 |
|---|---|---|
| 节点名 / compatible | `sdhci0` / `arasan,sdhci-8.9a` | `board_devices.c:168-170` |
| 寄存器基址 | `0xE0100000` | `board_devices.c:170`、`platform.h:171` |
| SDIO 时钟 | `100000000` Hz | `board_devices.c:34` |
| 中断号 | `PLAT_IRQ_SD0 = 56` | `platform.h:74` |

驱动写法照移植侧既有驱动（`axi_gpio.c` / `uart_ps.c`：C + 从描述表取基址），
注册成 `device_t` 块设备后，**上一轮刚落地的 `blk_device_read/write` 与 FATFS 的
`diskio` 就都能走它**（`diskio.cpp` 要的只是"一个能按偏移读写的 VFS 节点"）。

#### ★ 判据必须分成两半，而且先做不依赖卡的那一半

| 半 | 内容 | 能否今天验 |
|---|---|---|
| **① 控制器** | 软复位、Capabilities 读回、SDCLK 编程（100MHz 分频）、电源/中断寄存器写回读 | ✅ **能**（不需要卡） |
| **② 卡** | CMD0/CMD8/ACMD41 初始化、CMD2/CMD3/CMD9、CMD17/24 的**扇区读写往返** | ❌ **必须插卡** |

★ 顺序不能反：① 失败时不该去怀疑卡；② 失败时也必须能分清"控制器没配好"
还是"卡没响应"。这条纪律与 M4A-1.4 那次"先 RAM 盘后真设备"是同一条思路。
