...
/*
 * ★ D7:C 处理函数**入口**的 SP 不是 8 字节对齐的次数 ★ 必须恒为 0。
 *
 * 异常帧的位置是 `S - 64`(S = 被中断那一刻的 SP),所以帧会**继承 S 的对齐** ——
 * 而 S 合法地可以是 4 mod 8(被中断的代码在 libgcc 的 Thumb 函数
 * `__udivmoddi4` 里,`push {r4,r5,lr}` 是 12 字节)。于是 `bl` 进 C 时 SP 也不对齐,
 * 形式上违反 AAPCS。
 *
 * 修法:`_vec_irq`/`_vec_svc` 在 `bl` 之前 `bic sp, sp, #7`(**帧不动** ——
 * `EXC_FRAME_LEAVE` 靠 `帧基址 + 64 == S` 还原 SP,挪帧会断掉这个等式)。
 * 本计数直接量那一步有没有生效,不靠推理。
 */
