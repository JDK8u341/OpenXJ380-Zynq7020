 ...
 *
 * 这是区分"驱动/寄存器配置有问题"与"外部线缆/对端有问题"的唯一手段 ——
 * 两者的现象完全一样(收不到数据),排查方向却相反。
 *
 * 内部环回(把 MR[7:6] 设成 0b10)时,发送端在芯片内部直接接到接收端,
 * 完全不经过外部引脚。所以:
 *   环回收得到 -> UART 与驱动都正常,问题在外部通路;
 *   环回收不到 -> 问题在驱动或寄存器配置,与线缆无关。
 *
 * ⚠ 会临时改 MR 再改回。只能在启动阶段(没有别人在用这个串口时)调用。
 *
 * @return true = 环回成功
 */
