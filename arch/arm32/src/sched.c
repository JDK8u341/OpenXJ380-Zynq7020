...
/* ------------------------------------------------------------------ */
/* ★ 破坏性 A/B:严格非抢占(把 current 粘住)★                       */
/* ------------------------------------------------------------------ */

/*
 * ★ M4-8.5:无饥饿判据的对照组 ★
 *
 * **这是一个计数,不是布尔** —— 含义是"还允许连续粘住 current 多少次"。
 * 每粘一次减一,减到 0 就恢复正常选取。生产路径上恒为 0。
 *
 * 为什么必须是计数而不是"置 1 就一直粘":粘住之后**别的线程一次都跑不到**,
 * 包括那个负责收尾的监视器和 kmain 自己 ⇒ 没有任何人能把它清零,
 * 系统就地挂死,对照组什么也观察不到(那不是"判据不承重",是**测不了**)。
 * 用计数表达"粘住一段有限的时间",对照组才有开始与结束。
 *
 * 语义(M4-8 的行为等价物):**一个正在跑的真实线程,时间片到点也不被换下。**
 * 于是别的可运行线程永远轮不到 —— 这正是"无饥饿"要证伪的那件事。
 *
 * ⚠ 两处与计划原话的差别,都写在这里:
 *
 *   ① **idle 不受粘住影响**。idle 的 `status` 是 RUNNING,`sched_current_runnable`
 *      对它返回真(那处不对称是源 OS 的原样),所以"current 可运行就粘住"
 *      字面执行的话,**启动流程会把自己粘住** —— 4 个负载线程一个都起不来,
 *      监视器也不会跑,对照组观察不到任何东西。而源 OS 在 current 是 idle 时
 *      **本来就会切走**(它的 `timer_handle` 只在 `task_level != TASK_IDLE_LEVEL`
 *      时累积时间片),所以"排除 idle"才是 M4-8 的忠实等价物。
 *
 *   ② **粘住发生在扫描之后**(见 `sched_select_next`)。扫描里的唤醒照做,
 *      于是对照组只改了"选谁"这一件事,不改"会不会被唤醒"。
 *      这一点很要紧:被粘住的线程会处于"**已经可运行、却一直不被选中**"
 *      的状态 —— 那正是无饥饿判据要抓的形态。
 */
u32 g_pick_sticky;
...
tcb_t sched_select_next(const sched_queue_t *q, tcb_t current, u64 now)
{
    ...
    /*
     * 2b. ★ 破坏性 A/B:把 current 粘住(严格非抢占)★
     *
     * 位置是刻意的:**扫描已经做完了**(唤醒、平均值、fallback、idle 都算过了),
     * 只是**不用它的结果** —— 于是对照组与生产路径的差别**只有"选谁"**。
     *
     * 被粘住的线程照样会被唤醒(status 从 WAIT 变成 START),只是永远轮不到
     * 它被选中。这正是"无饥饿"要抓的形态:一个**可运行**的线程被无限期漏掉。
     */
    if (g_pick_sticky != 0u && current != NULL && current->task_level != TASK_IDLE_LEVEL &&
        sched_current_runnable(current)) {
        g_pick_sticky--; /* 有限的粘住时间,免得无人能收尾(见上面的说明)*/
        sched_mark_dispatched(current, now);
        return current;
    }
