...
static void dump_frame(const char *what, arm_irq_frame_t *frame, u32 extra)
{
    console_printf("\n!!! %s !!!\n", what);
    if (frame != NULL) {
        ...
        console_printf("  r12/lr   = 0x%08X 0x%08X\n", frame->r[12], frame->pc);
    }
    if (extra != 0) {
        console_printf("  附加信息 = 0x%08X\n", extra);
    }
    console_puts("  系统已停止。寄存器可在 JTAG 下用 rrd 读取。\n");
}

void c_undef_handler(arm_irq_frame_t *frame)
{
    dump_frame("Undefined Instruction", frame, 0);
}

void c_svc_handler(arm_irq_frame_t *frame)
{
    /* M1 阶段还没有用户态;走到这里说明有人主动发了 SVC */
    dump_frame("SVC (no syscall layer yet)", frame, 0);
}
...
void c_prefetch_abort_handler(arm_irq_frame_t *frame)
{
    ...
    console_printf("\n!!! Prefetch Abort !!!\n");
    console_printf("  IFAR = 0x%08X  (取指失败地址)\n", ifar);
    console_printf("  IFSR = 0x%08X  (状态:bit0=0 表示域故障,bit0=1 表示页故障)\n", ifsr);
    dump_frame("Prefetch Abort", frame, ifar);
}
...
void c_data_abort_handler(arm_irq_frame_t *frame)
{
    ...
    console_printf("\n!!! Data Abort !!!\n");
    console_printf("  DFAR = 0x%08X  (访存失败地址,对应 x86 的 CR2)\n", dfar);
    console_printf("  DFSR = 0x%08X  (状态码)\n", dfsr);
    dump_frame("Data Abort", frame, dfar);
}
