...
### 1.1 ★★ 为什么"生命周期"现在没做完 —— **不是漏做，是它本来就不在这个阶段** ★★

这一点以前只在 §2.5/§9.3 的证据里隐含，没写成一句话，容易被读成"还没实现的半成品"。说清：

> **源 OS 的内核线程没有生命周期。** 它们能被执行流标记成 `status = DEATH`，
> 但**没有任何释放路径**（§2.5 的可达性证明、§9.3 的五条旁证）——
> 不是"忘了写"，是那套释放链（`kill_proc` → `kill_proc0` → `kill_thread0`）
> 服务的是**进程 / 用户线程**，而内核线程挂在 `kernel_group` 上，被
> `Cannot kill System process.` 挡在门外。

> **而"释放"这件事真正存在的地方是用户态**：`kill_proc` / `kill_proc0` /
> `kill_thread0` / `reaper_thread` / 僵尸与 `wait4` —— 整条链都是围着
> **进程与用户线程**建的。
...
| | 状态 |
|---|---|
| 内核线程 | 退出路径**已按源 OS 落地**（`DEATH` → 让出 → `wfi`，D14/M4-11.2）；**不回收**，因为源 OS 就不回收 |
| 用户进程/线程的生命周期 | **还没有** —— 因为**我们还没有用户态** |

⇒ **生命周期（创建 → 运行 → 退出 → 回收 → 复用栈与 TCB）要在用户态那个阶段一起做**，
那时源 OS 的回收链**才有合法用户**（现在把它接进来也没有东西可回收，
只会在 `kernel_group` 上多一个源 OS 里不存在的回收器）。
★ 用户态的归属 **2026-09-18 已拍板（合流决策 D4）**：**新增 M4A-4「用户态与 syscall 层」**，
放在 M4A-3 与 M5 之间 —— 见计划 §0.5.7 末尾。
⚠ **但 M4A-4 只到"能跑一个手写的静态 32 位用户程序"**，不含 busybox/musl
（「跑 `shell.elf`」已随 D4 移给 M7）；而下面第 4 条"内核栈从固定槽池换成按需分配 + 归还"
需要的是**进程真的会被拆**，也就是 M7 那条链 —— **M4A-4 时不要顺手把池改成可回收**。

⚠ 这不是对 §4.0（D14"不回收"）的反悔：**D14 只管内核线程**。用户态那套回收是源 OS
**本来就有**的东西，照它做属于"照抄"，不违反"不发明源 OS 没有的机制"。
...
> ★ **一句话结论（这就是"内核线程没有生命周期"的出处）**：
> 源 OS 的内核线程**只有第一段**（标记自己为死），**永远没有第二段**（释放）。
> 换句话说 —— **内核线程没有生命周期**：它可以"停"，但不会"结束"。
> 释放栈与 TCB 的那条链（`kill_proc` → `kill_proc0` → `kill_thread0`）
> 是给**进程 / 用户线程**用的。详见下面 129-145 行的可达性证明，以及 §9.3。
...
>
> ★ 换个说法（更贴近本质）：**源 OS 里内核线程没有生命周期** ——
> 第一段（把自己标成 `DEATH`）可达，第二段（释放）**对内核线程这一整类不可达**。
> 那套释放链服务的是**进程 / 用户线程**。所以我们没有用户态之前，
> "生命周期"这件事**没有可做的地方**（详见 §1.1 与 §4.4）。★
...
| 释放栈/TCB | `kill_thread0`（唯一路径，被 `Cannot kill System process.` 挡住）| ❌ 无 | ★ **内核线程不做**（D14：源 OS 本来就不回收内核线程）；**用户态那套要做** —— 等用户态一起（§1.1 / §4.4）。代价是池只增不减 ⇒ 池 32→64 + 容量判据（实测峰值 33）|
| 回收者 | `reaper_thread`（只扫进程；且僵尸不在它的条件里，链本身是断的）| ❌ 无 | ★ **现在不做**：它扫的是**进程**，我们没有进程可扫 ⇒ 与用户态一起做（§4.4）。**不是"永久不做"** |
...
### 4.4 ★★ 生命周期为什么"没做完"：源 OS 的内核线程没有生命周期，而用户态还没做 ★★

这一节只回答一个问题：**"线程生命周期"这件事现在算完成了吗？**
答案是：**内核线程这条线已经按源 OS 做完了（源 OS 就到这儿为止）；完整的生命周期要等用户态。**

#### 4.4.1 事实（都已核实，出处见 §2.5 / §9.3）

| 事实 | 出处 |
|---|---|
| 源 OS 的内核线程能被标成 `DEATH`，但**没有任何释放路径** | §2.5（`kill_proc` 对 `kernel_group` 直接 return）、§9.3（五条旁证） |
| 那套释放链（`kill_proc` → `kill_proc0` → `kill_thread0`）服务的是**进程 / 用户线程** | §2.5 第二段、§8.9（进程级生命周期） |
| `reaper_thread` 遍历的是 `kernel_group->child_pcb`（**进程**），不是线程队列 | `reaper.cpp:35`/`:48-61`；§9.3 旁证 1 |
| 僵尸（`ZOMBIE`）**不满足** reaper 的 `status == DEATH` 条件 | §9.3 旁证 3 |

⇒ 所以"内核线程退出后不释放"**不是我们偷懒，是源 OS 的既成语义**。
D14 的决定不是"少做一步"，而是**不多做一步**。

#### 4.4.2 为什么现在不该做

完整生命周期 = 创建 → 运行 → 退出 → **回收** → 栈与 TCB **复用**。
最后两步在源 OS 里是**围着进程建的**：

```
用户进程退出 → ZOMBIE → wait4 / kill_proc → kill_proc0
                                              ├─ kill_thread0(每个线程) → free(thread)
                                              └─ free_frames(内核栈 / syscall 栈)
                              reaper_thread: kernel_group->child_pcb 里挑 DEATH 的进程
```

而**我们还没有用户态**（M7）。把它现在接进来会变成：

* 没有进程可扫 ⇒ `reaper_thread` 是个**空转的循环**；
* 没有"用户线程"可杀 ⇒ `kill_proc` 那条链只会在 `kernel_group` 上被拒；
* 唯一"有东西可回收"的对象是**我们自检用的内核探针线程** ——
  而源 OS 明确**不回收**这一类（§2.5）。

⇒ 结论：**现在做 = 在源 OS 里发明一个它没有的回收器**，正好违反 §4.0 的"不发明"。
**放到用户态做 = 照抄源 OS 本来就有的那条链** —— 同样是"尊重源 OS"，两者不矛盾。

★ 一句话：**D14 的"不回收"只管内核线程；用户态的生命周期是要做的，只是得等用户态。**

#### 4.4.3 什么时候做、做完长什么样

* **时机**：**M4A-4「用户态与 syscall 层」**（2026-09-18 拍板，合流决策 D4；见计划 §0.5.7 末尾）。
  它天然应当**与 syscall / 进程模型同期** —— 因为 `exit` / `wait4` / 僵尸就是 syscall 面。
  ⚠ 但**下面第 4 条要等到 M7**：M4A-4 的判据只是一个手写的静态用户程序 + `write`/`exit`，
  还没有"进程会被拆"这件事；把栈池改成可回收要有 `kill_proc0` 真正在跑才算验过。
* **做的内容**（对着源 OS 抄，不是设计新东西）：
  1. 用户线程退出 → `ZOMBIE`，父进程 `wait4` 收尸；
  2. `kill_proc` / `kill_proc0` 拆进程时逐个释放线程栈与 TCB；
  3. `reaper_thread` 接上（注意 §9.3 旁证 2：`kill_proc_deferred` 在源树里是**死代码**，
     接线时要决定"照它接"还是"照僵尸路径接"—— 这是个**要问作者**的点）；
  4. 内核栈从**固定槽池**换成"按需分配 + 归还"，`kstack_peak_used` 那套判据继续留着当回归。
* **届时 §5 那张耦合项清单**就是入口清单（1/2/5/6/7 会**重新变得需要处理**，
  因为"线程真的会被释放"这个前提回来了）；全局 `scheduler_lock` 也是那时才需要（§3 表）。
* **现在要守的约束**（在用户态做好之前，一直有效）：**池只增不减**
  ⇒ 内核线程数有硬上限，`kstack_peak_used` / `kstack_headroom` 每次启动都打出来（§4.0 事项 2）。

#### 4.4.4 一句话现状（对外说的时候用这句）

> 内核线程按源 OS 的语义做完了 —— 包括"退出后不释放"。**完整的线程/进程生命周期
> （退出 → 回收 → 复用）依赖用户态**，我们会做，但要等用户态那个阶段，
> 因为那套回收链在源 OS 里就是围着进程与用户线程建的。
...
> ★ **这张表什么时候会重新全亮**：**用户态把生命周期接上之后**（§4.4）——
> "线程真的会被释放"这个前提回来了，1/2/5/6/7 会重新变成必须处理的耦合，
> 全局 `scheduler_lock` 也是那时才需要（§3 表）。所以它不是废纸，是**用户态阶段的入口清单**。
...
⇒ **再往上一层的措辞（这才是不容易看出来的那一层）**：
> **源 OS 的内核线程没有生命周期** —— "标记为死"可达，"释放"对**内核线程这一整类**不可达。
> 这条释放链是给**进程 / 用户线程**建的，所以**我们还没有用户态之前，
> "生命周期"没有可做的地方**。完整的生命周期与用户态同期做，见 **§1.1** 与 **§4.4**。
> （因此 §3 表里"释放栈/TCB""回收者"两行的"不做"是**暂时**的，不是决定永久不做。）
...
### 9.8 这次扫描对 M4-11 的影响（当时结论：**方案 A 不变，但要补三件事**）

> ⚠ **标题是当时的结论，已被 §4.0 覆盖** —— 拍板后**方案 A 作废**（不引入回收者）。
> 下表里 **1 / 2 / 3 三条都是"回收者"的要求，随方案 A 一起搁置**；
> 它们不是废纸：**用户态把生命周期接上时，这张表就是要照的清单**（§4.4.3、§5）。
> 只有 **4（`mutex->owner` 不变量）在 M4-11 真的做了** —— 落地形式是退出路径上那条绊线
> （`kthread_exit_held_lock` / `kthread_exit_lock_guard`，见 §4.3 方案 B 的判据）。
> 第 5、6 条与回收无关，仍然有效。
...
> ⚠ **本节是第二批扫描（2026-09-13）当时的结论快照。** 之后 §4.0 拍板"**不改源 OS 行为、
> 方案 A 作废**"，所以下面"没有改变"里的**第 2 条已不成立**（回收者不做了），
> 第 1 条的措辞也要按 §4.4 收窄：**"退出"是必需，"回收"要等用户态**。
> 以 §4.0 / §4.3 / §4.4 为准，本节只作留档（也保留"当时是怎么判的"这条线索）。
