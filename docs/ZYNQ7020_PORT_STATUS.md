...
> （释放链 `kill_proc` → `kill_proc0` → `kill_thread0` 是给**进程 / 用户线程**建的，
> 内核线程挂在 `kernel_group` 上会被 `Cannot kill System process.` 拒掉）。
> 所以我们这边**内核线程这条线是照源 OS 做完了**（不回收 = 不多做一步）。
> **完整的生命周期（退出 → 回收 → 复用栈与 TCB）依赖用户态**，会在用户态那个阶段做 ——
> 那时源 OS 的回收链才有合法用户。**这不是"半成品"，是顺序。**
...
| 心跳 | OCM `0x00020000` 起 **64 槽**（已分配 0–31，32–63 是启动时清零的保留区）；故障注入选择器 **`0x00020100`**（2026-09-18 合流决策 D5 由 32 槽 / `0x00020080` 扩容而来，已上板验证）|
