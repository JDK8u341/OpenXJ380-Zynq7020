...
**M0–M4-10 全部完成并板上验证。当前板级自检 86 passed / 0 failed。**
...
**下一步 = ★ task 系统重新规划 + M4-11（mutex + 线程退出/回收）★**
⇒ **先读 `docs/PTASK.md`**（task 子系统专题分册：源 OS 的调度/线程/生命周期事实、
我们与它的差异、待作者确认的 5 个问题、方案与判据）。

★ 为什么需要"重新规划"：调研发现**源 OS 的内核线程退出之后不释放内核栈与 TCB**
（`kill_thread` 里那句 `kill_thread0` 是注释掉的，另一条释放路被 `kill_proc` 的
`"Cannot kill System process."` 挡死），而**我们的自检探针是"用完就死"的临时线程**
（源 OS 的内核线程基本是永久基础设施）⇒ 这条**用法差异**使得"退出 + 回收"在我们这边
**不是可选优化而是必需**（栈池只有 32 槽）。细节与证据链全在 `PTASK.md`。
...
| **PTASK.md：task 子系统专题分册 + 重新规划** | 本次 | 见 `docs/PTASK.md`；坑表 45→47 |
...
> 生命周期事实（全部带 `文件:行号`）、我们与它的差异表、待作者确认的 5 个问题、
> 方案 A/B 与判据、以及**必须一起处理的 7 条耦合项**。下面只是摘要。

**为什么要"重新规划"**：M4-11 调研发现两处与计划原文不符，而且第二处牵连到整个
线程生命周期模型：

1. **D13 的正确处置是"换成源 OS 的 yield mutex"**（不是"可睡眠 mutex"——源 OS 没有
   可睡眠互斥；`mutex.cpp:21-22` 的作者注释明说"不把线程切到 WAIT"）。
2. ★ **源 OS 的内核线程退出之后不释放资源**：`kill_thread` 只置 `DEATH`
   ...
   ⇒ 而**我们的自检探针是"用完就死"的临时线程**（源 OS 的内核线程基本是永久基础设施）
   ⇒ 这条**用法差异**使得"退出 + 回收"在我们这里**必需**（栈池只有 32 槽，
   `kstack_headroom` 已经贴边）。

| 步 | 内容 | 判据 |
|---|---|---|
| **0** | 把 `PTASK.md` §4.2 的 5 个问题（Q0/Q0b/Q1/Q2/Q3）发给上游作者；**无论怎么答，回收照做**（用法差异是既定事实）| 答复决定"偏离清单怎么写"，不决定"做不做" |
| **11.1** | `mutex`（源 OS yield 语义）：纯状态机放纯逻辑层 + 钩子，宿主穷尽测；**`console_excl` 换成它（D13 结案）** | 板上：`sched_off_total_ns` 几乎不再增长 ⇒ 饥饿监视器 `skipped` 掉到 ~0 |
| **11.2** | **线程退出 + 回收**（方案 A：垂死线程"置 DEATH + 让出 + wfi"，回收者"摘节点 + 还栈 + 还 TCB"）+ `remove_task` + 全局 `scheduler_lock`（**D14 结案**）| `used_slots` 回落、名册峰值后下降、`kstack_headroom` 变大；第七组 A/B（关掉回收 ⇒ 槽不回落）|
| **11.3** | `sched_park_self` 退场（源 OS 没有它）+ 文档/偏离清单 | — |

⚠ **做 11.2 时必须同时处理 PTASK.md §5 的 7 条耦合项**，其中两条会让已验证的东西坏掉：
对照组的 ctx 快照/还原（前提是"窗口内无线程创建/销毁"）与清理里的 `ca->status` 写入
（都可能 UAF）⇒ 对照组窗口内**显式暂停回收**。另外：`mutex->owner` 在源 OS 里不注销，
我们一引入线程退出它就变得可达 ⇒ 必须一起处理或记退化。
...
| 项 | 状态 |
|---|---|
| ★★ **task 系统的线程生命周期需要重新规划** ★★ | 源 OS 让"退出"与"释放"分开、而且**第二段对内核线程不可达**（见坑 47 与 `docs/PTASK.md`）。我们的自检探针是**临时**线程（源 OS 的内核线程基本永久）⇒ **用法差异**使得回收必需。方案 A（两段式 + reaper 覆盖内核线程，记偏离）已写进 `PTASK.md` §4.3；**待办：把 §4.2 的 5 个问题发给作者**，答复只决定"偏离怎么写"，不决定"做不做" |
| ★ **BSP idle 的 `task_level` 在源 OS 里漏赋值** ★ | `main.cpp:520-541` 从不赋 `task_level` ⇒ 保持 memset 的 0 = `TASK_KERNEL_LEVEL`（不是 `TASK_IDLE_LEVEL`）⇒ ①`kill_thread` 不拒绝它 ②`is_task_schedulable` 不排除它 ⇒ 它作为候选被选中时因 `context0.rip == 0` 而**放弃切换**（白做一次派发）③被 EEVDF 计费。**我们两个 idle 都显式设 1** ⇒ 与源 OS 意图一致、与其 BSP 实际行为不同 ⇒ **已记入偏离清单**（见 §4.5 的 M4-11 调研与 PTASK.md §2.4）|
