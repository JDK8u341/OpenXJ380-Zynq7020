...
| 前置 | 内容 | 为什么必须排在 B 之前 |
|---|---|---|
| **P1** | **修掉 `irq_spurious = 1285`**（当前唯一的红项） | 不允许带着一个**未解释**的失败进入下一阶段。它会污染之后所有"这个现象是不是 spurious 引起的"判断 —— 而 P2/B 阶段恰好全是中断密集的改动 |
| **P2** | **UART / GIC / 定时器变成可描述设备** | 否则 DTB 里十来个节点只有 1 个驱动能匹配，"换 DTB 不重编译"这件事根本验证不充分。而且描述表当前是从 `AXI_GPIO_1` 那份 BSP 生成的，**里面连 UART 条目都没有**，与实际运行的 PS 配置对不上 |

**已拍板的执行顺序（2026-09-13）**：

```
P1  修掉 irq_spurious        <- 当前唯一红项
M4  把 fs 搬完               <- 见 §4.2 M4
P2  UART/GIC/定时器 可描述
B1..B6                       <- 本节
```

M4 排在 P2 之前：P2 要把外设收进描述层，而描述层最终要落到 `device_t`/devfs 上，
**fs 没搬完之前做 P2 会返工**。B5 更是直接依赖 M4 的成果（`regist_device` 的语义就是 devfs）。
...
### 4.5 M4 范围与依赖排序（把 fs 搬完）

#### 先看清量级：行数会骗人

| 项 | 体量 | 说明 |
|---|---|---|
| `driver/fs/` + `include/fs/` 合计 | 26 文件 / 2634 KB / **~34088 行** | 看起来很吓人 |
| ├ `ffunicode.cpp` | **1992 KB** | FATFS 的 Unicode 转换表，**生成物**，可移植 |
| ├ `ff.cpp` | 315 KB | FATFS 核心（ChaN FatFs），**本来就是可移植 C** |
| └ VFS + 各种特殊 fs | ~215 KB | `vfs.cpp` 46.5、`procfs` 49.9、`unixsock` 22.9、`pty` 19.5、`nmfs` 18.2、`socketfs` 17.6、`dnsfs` 12.2、`tmpfs` 10.3、`dev.cpp` 9.5、`pipefs` 7.9 |
| `driver/device.cpp` | 17 KB | 设备管理器 |

**真正的"移植工作量"远小于行数**：2 MB 是数据表，315 KB 是现成的可移植 C。
工作量集中在 **VFS 胶水层**和**它脚下那套基础设施**。

#### 依赖盘点：fs 站在什么上面

按实际 include 统计（文件数 / 出现次数）：

| fs 需要的东西 | 用量 | ARM 侧现状 |
|---|---|---|
| `malloc` / `kfree`（**堆**）| 12 文件 / 65 处 | ❌ 没有 |
| `mutex` | 5 文件 / 39 处 | ❌ 没有（有 `spin_t`）|
| `task` / `current_task` / `schedule` / `poll`（**调度器 / PCB**）| 7 文件 / **204 处** | ❌ 没有 |
| `syscall` 层 | 4 文件 / 5 处 | ❌ 没有（`_vec_svc` 只做诊断）|
| `regist_device` / `device_manager` | 2 文件 / 5 处 | ❌ 没有 |
| `krlibc`（字符串/内存函数）| 全部 | ❌ 没有（freestanding）|
| `mm/uaccess` | `device.cpp` 直接 include | ❌ 没有 |
| `dlinker`（.sys 模块 ABI）| 2 文件 / 2 处 | ❌ 没有 |

`vfs.cpp` 的 include 里有 `task/pcb.h`、`task/poll.h`、`syscall/syscall.h`、
`rtc.h`、`rng.h`、`pipe.h`、`cpu/lock.h`、`llvm` 与 `list.h`。
**也就是说：不先把堆、锁、PCB 这三样立起来，VFS 一行都编不过。**

#### 子阶段与排序

排序原则：**每一步都必须是"上板可验证"的**，而且尽量让最少的依赖解锁最多的东西。

| # | 内容 | 依赖 | 上板可验证方式 |
|---|---|---|---|
| **M4-1** | `krlibc` 子集 + `errno`（纯函数）| 无 | **宿主单测**即可（不需要上板）|
| **M4-2** | **内核堆** | M4-1 | 分配/释放压力 + 碎片统计自检；心跳槽记失败数 |
| **M4-3** | 互斥与等待原语 | M4-2 | 沿用已有的 spinlock 互斥压力测试（两核）|
| **M4-4** | **单任务 PCB** + `current_task` | M4-2/3 | 任务自检 + 心跳可见 |
| **M4-5** | device manager + `regist_device` + devfs 骨架 | M4-1..4 | `regist_device` / `get_device` 往返 |
| **M4-6** | **VFS 核心 + `tmpfs`** | M4-5 | **第一个完整切片**：建/读/写/列目录全在内存里，不需要任何块设备 |
| **M4-7** | 块设备驱动（SD/arasan）+ `diskio` | 独立，可并行 | 读写扇区 + 写回读 |
| **M4-8** | **FATFS 接上**（先 RAM 盘，后 SD）| M4-6 + M4-7 | 挂载 + 读写文件 + 与主机侧比对 |
| **M4-9** | `procfs` / `dev` / `pipe` / `pty` | M4-6 | 逐项 |

**M4-6 是关键节点**：它是第一个"不依赖任何未移植硬件"就能上板验收的完整切片。
在此之前每一步都能单独验证，但只有到 M4-6 才第一次出现**用户可见的文件系统**。

#### 明确推迟的东西（不在 M4 范围内）

| 项 | 为什么推迟 |
|---|---|
| `socketfs` / `unixsock` / `dnsfs` / `nmfs` | 属于网络栈，依赖 lwIP 与 socket 层 —— 那是 M5+ 的事，且 fs 本身的移植完全不需要它们 |
| `pty` | 需要 tty 子系统 |
| `dlinker`（`.sys` 可加载模块）| 是**另一条独立线索**，见 B5 的边界说明 |
| 真正的调度器 / SMP 调度 | M4 只需要**单任务 PCB**，让 `current_task` 有东西可返回 |
| `mm/uaccess` | 还没有用户态。M4 期间以**退化实现**顶上，并标明它不是最终形态（见下方风险）|

#### 三个需要先拍板的决策点

**决策 1：堆用哪个？**
x86 侧用的是外部依赖 `liballoc-x86_64.a`（`AGENTS.md`：`ninja all` 会去下载它）。
ARM 侧有三条路：

| 方案 | 优点 | 代价 |
|---|---|---|
| (a) 为 ARM32 构建 liballoc | 与 x86 行为一致，长期最省事 | 引入 Rust 构建链，且当前 ninja 图里没有 |
| (b) 自己写一个简单分配器（空闲链表 / 伙伴）| 纯 C，**宿主可单测**，无外部依赖 | 与 x86 行为不完全一致；碎片表现不同 |
| (c) 先用固定 arena 的分段分配 | 最小 | 只能应付启动期，迟早要换 |

**建议 (b)**，理由与本项目一贯做法一致：判定逻辑（切分/合并/对齐/越界）是纯逻辑，
能在宿主机上把最容易错的地方钉死；而且它不需要把 Rust 工具链拉进构建图。
**但要在计划里写明**：这与 x86 侧不是同一个分配器，将来若要统一需另做一轮。

**决策 2：`mutex` 在没有调度器时是什么？**
x86 的 `mutex` 是自旋 + 睡眠等待队列。没有调度器时只能退化成**纯自旋**——
在双核上**正确但会浪费 CPU**，而且如果临界区里等待某个只能由别的任务满足的条件就是死锁。
**建议**：M4-3 提供 `kmutex_t`，语义定义为"自旋 + 可选的让出钩子"，并在文档里
**明确标注"当前不能睡眠"**。等 M4-4 之后若要真正睡眠，接口不变、实现替换。

**决策 3：FATFS 的块设备从哪来？**
`sdhci0` 已经在设备描述表里（`arasan,sdhci-8.9a`），但**驱动一行都还没写**，
而且 SD 的初始化（电压切换、ACMD41、高速模式）是实打实的硬件工作。
**建议**：M4-8 先接 **RAM 盘**——它与真实块设备走完全相同的 `diskio` 路径，
只差最后的硬件；这样 FATFS 的移植可以**独立于 SD 驱动**先验证完，
而 M4-7 并行推进。等两边都好了再合。

#### 与 B 系列的关系（为什么 B 在 M4 之后）

B5 的难点正是 **`regist_device` 的语义依赖 devfs**。而 devfs 是 M4-5/M4-6 的产物。
**M4 做完，B5 的 `regist_device` 才有真实语义可对齐**，否则只能走 B5-b 那种
"注册了却访问不到"的退化实现。这就是 §4.4 把 M4 排在 B 之前的实质原因。

另外 M4-9 的 `dev` 文件系统正好是**块设备与字符设备对上 `device_t` 的地方** ——
它同时是 B5"驱动接口兼容旧 XJ380"的第一个真实用户。

#### 最大的风险

**不是代码量，是"退化实现悄悄变成完成态"。**

M4 里至少有三处会做退化版：`mutex` 不能睡眠、`mm/uaccess` 没有用户态、
`current_task` 只有一个任务。这三处**都能让上层代码编过、跑起来、测试也过**，
但它们**不是最终形态**。如果不在每一处都留下"这是退化实现 + 何时替换"的标记，
它们会变成后面每一层默认接受的前提，而那种债务在 M7（用户态）才爆发。

**所以 M4 的每一处退化都必须在 README 里列进一张"退化实现清单"**，
写清：退化的是什么、为什么现在可以退化、什么条件下必须替换。
