...
**M4-6 的寄存器帧部分已完成**（提交 `451e252`）：按本板实测改正了 pc 的偏移，
板级自检 **43 passed / 0 failed**。
M4-6 的调研部分在三份清单里（§4.7）。
**下一步是 M4-7：上下文切换（ARM 汇编，per-CPU）** ——
它的第一件事是按 §4.5 把中断路径用 `srsdb` 落到任务的栈上。**
...
| M4-6 寄存器帧 | `451e252` | **43 passed / 0 failed**（新增 `exc_frame_layout`）；实测改正 pc 偏移（SVC 与 Data Abort 各偏 4）；两组 guard A/B 仍成立 |
