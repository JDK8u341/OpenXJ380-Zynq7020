...
### 0.5.0 ★ 对外读者看哪里（2026-09-13：仓库准备上传到 fork）★

本分支即将作为 fork 公开，所以**入口文件重排过一次**。规矩是：

| 读者 | 先读 | 说明 |
|---|---|---|
| 拿到 fork 的外人 | `README.md` → `docs/ZYNQ7020_PORT_STATUS.md` | 根 README 现在**开头就是"WIP / 不可用"声明**（上游 README 原文逐字节备份在 `OLD_README.md`）；STATUS 讲清"能做什么/不能做什么/改了什么/怎么构建/怎么测" |
| 想知道"将来怎么与上游合流" | `docs/ZYNQ7020_INTEGRATION_PLAN.md` | ★ **哪些层复用上游代码、哪些 ABI/API 保持不变、`arch/arm32` 怎么接进原有上层抽象、分阶段路线与待拍板决策（D1–D6）** ★ |
| 接手继续开发的人（我/下一次会话） | **本文件 §0.5.1 → §0.5.3 → §0.5.4 → §0.5.5 → §0.5.7** | 本文件是**内部工作文档**（含 54 条坑表与逐阶段判据），比对外那两份详细得多，但**不要求外人读** |
| 想知道 task/线程那些决定 | `docs/PTASK.md` | 带 `文件:行号` 的证据链 |
| 想知道某个 ARM 模块为什么这么写 | `arch/arm32/README.md` | 模块级设计与踩坑 + 退化/偏离清单 |

⚠ 两份对外文档里的话**必须与代码事实一致**，凡是"已完成"都标了准确数字
（97 项自检、8 组 A/B、9 failed/74 passed 宿主、`git diff 08e5c9c` 的机械判据）。
以后每完成一个阶段，**那两份文档要跟着改**（它们在 README 的索引里，是被引用的）。

★★ **根 README 开头还有一份"AI 生成声明"**（2026-09-13 应作者要求加）：写明
**生成者**（DeepSeek Harness + DeepSeek Flash 4.1）、**AI 做了什么**（本 fork 新增内容的
全部：写码/调试/单元测试/上板验证/判据设计/文档，并给出各部分的实测行数）、
**人做了什么**（方向、取舍决策、硬件）、以及 ★ **没有任何一行代码经过人类逐行评审** ★，
并且点明"判据本身也是 AI 设计的 ⇒ 只能读作 **AI 自证**"。
这份声明**是需要维护的**：每完成一个阶段要更新行数与已知坑数；
**将来若真的有人开始逐行评审，必须把那份声明改写成事实**（不能让它变成过期的好话）。
同样的声明抄在 `docs/ZYNQ7020_PORT_STATUS.md` §0 里。
...
**★ M4A-1.1a / 1.1b / 1.2a / 1.2b / 1.4 全部完成,1.5 的 `dev` 与 `pipe` 也已进图并上板验证**
—— 上游 VFS、FATFS、devfs、pipefs 都在板上跑起来了;pty 也已**起搏**(主/从两个方向各做了一次真实往返):板上 **148 passed / 0 failed**,
`ninja -f build-arm.ninja arm32` = **0 个未定义符号**。**
★ **devfs 骨架已退场,M4A-1.1b 那条退化项结案**:换用上游 `driver/fs/vfs/dev.cpp`
(被迫二选一 —— 骨架与上游都定义 `devfs_register`/`devfs_delete`,而链接带 `-z muldefs`),
顺带抓到并修掉一个**潜伏的"幽灵设备"** bug(上游把清理写在了局部拷贝上,见坑表 59)。
★ **1.2b 的验收判据"建/读/写/列目录"已上板**(十条判据,含**反面控制**:在未挂载的根 fs 上
做同样的往返必须不成立 —— 上游给未挂载节点的回调是 `empty_func`,"调用成功"不携带信息)。
★ **1.4 的验收判据"挂载 + 读写文件 + 与主机侧比对"也已上板**:FATFS 在 8MB RAM 盘上
格式化/挂载/建文件/写 32 字节/读回逐字节相同;宿主机用自己的解析独立核引导扇区,
并由算术反推子类型 FAT12。
★ **管道的验收也补上了(板上 136/0)**:内存往返 + 两个独立观测点
...
| **M4A-1.2b 链接缺口 11 → 0（落地层进程切片 + 格式化器三入口合一）** | `da645bf` `f8e57d0` `6120e90` | **39/39、0 未定义符号**；板上 **100/0**、八组 A/B 全检出（退出码 0）；宿主 `test_arm32_console.py` 7 条 + 10 子判据 |
| **★ M4A-1.2b 验收：上游 VFS 在板上跑起来（起搏 + tmpfs 建/读/写/列目录）** | 本笔提交 | **110/0**、八组 A/B 全检出（退出码 0）；十条新判据 `vfs_bringup` / `vfs_tmpfs_mounted` / `vfs_create` / `vfs_write=32` / `vfs_read=32` / `vfs_roundtrip` / `vfs_list_delta` / `vfs_list_before=0` / `vfs_cwd_root` / `vfs_negative_ok`；宿主 `test_arm32_vfs.py` 9 条 + 27 子判据 |
| **★ M4A-1.4：FATFS 在 8MB RAM 盘上跑起来（挂载 + 读写 + 与主机侧比对）** | 本笔提交 | **127/0**、八组 A/B 全检出（退出码 0）；19 条新判据（`fatfs_*` 13 条 + `fatbs_*` 5 条 + 卷大小）；宿主机独立解析引导扇区（`verify_fat_bootsector.py`），**由算术反推子类型 = FAT12**；宿主 `test_arm32_fatfs.py` 8 条 + 39 子判据（含把 `mktime` 编到宿主上与 `datetime` 逐日期比对） |

| **★ M4A-1.5（dev 部分）：上游 devfs 取代移植侧骨架 + 抓到"幽灵设备"** | 本笔提交 | **127/0**、八组 A/B 全检出（退出码 0）；`device_devfs_ab` 改用**上游自己的语义**（`pre-mount=1 post-mount=1 after-delete=1`）；`device_roundtrip` 新增"删了就是空了"判据并**据此修掉一个潜伏 bug**；宿主 `test_arm32_device.py` 从"骨架时代"更新到"上游 devfs 时代"（9 passed） |
| **★ M4A-1.5（pipe 部分）：管道的读写往返上板（136/0）** | 本笔提交 | 九条新判据 `pipe_setup` / `pipe_create` / `pipe_write=32` / **`pipe_fill_after=32`** / `pipe_read=32` / `pipe_roundtrip` / **`pipe_fill_drain=0`** / `pipe_short_write=8` / `pipe_short_read=8`；宿主 `test_arm32_vfs.py` 11 条 + 50 子判据（含一条**顺序检查**：每次读之前必须有写 ⇒ 把"不许读空管道"钉成机器可判） |
...
| 56 | ★★ **`va_list` 传值 = 游标推进在返回时丢掉 ⇒ 从第二次转换起全部读同一个槽位** ★★ | 宿主单测当场抓到：`console_printf("%d|%s", 42, "ok")` **访问违例**（`%d` 在子函数里取走 42，但拿的是自己的副本，回到格式化循环后游标没动 ⇒ `%s` 把 42 当指针解引用）；`"%llx|%d", 0x1122334455667788ull, 42` 打出 `1432778632`，正好是那个 64 位量的**低半部分**。★ 关键认识：**这不是宿主特有的** —— ARM EABI 的 `va_list` 是 `struct __va_list { void *__ap; }`（结构体，传值即复制），Windows x64 的是 `char *`（传值同样是复制），**两种 ABI 下都会丢** ⇒ 这个写法在**目标板**上一样会打出错值。⇒ `take_unsigned`/`take_signed` 的形参改成 `va_list_t *`。★ 意义：这是"宿主单测能在上板前拦下目标 bug"的第一个实例，而不是"宿主与目标行为不同" |
| 57 | ★★ **判据里写死"我以为的事实"= 假 FAIL；而"假 FAIL 的代价"是让人去改本来对的代码** ★★ | M4A-1.4 第一次上板 **126 passed / 1 failed**，失败项是**我自己加的** `fatbs_fat16_type` —— 我假定"用 `FM_FAT` 格式化出来就是 FAT16"。真相：`FM_FAT` 只说是 **FAT12/16 族**，子类型由 FatFs **按簇数自己选**（`ff.cpp:6718` 附近的重试逻辑），8MB 的卷上实测选到 **FAT12**（8 扇区/簇 ⇒ 2043 簇；FAT 占 7 扇区，而 FAT12 需要 6、FAT16 需要 8 —— **算术就能定**）。<BR>★ 同一批还查出**宿主脚本自己的**一个判据 bug：把 510/511 的 `55 AA` 按小端 u16 比较（`(bs[510]<<8)|bs[511] == 0xAA55`），于是对**正确**的卷报了 FAIL。<BR>⇒ 教训两条：① 判据只能断言**被测系统的契约**，不能断言"我以为它会怎么选"；能由产物**算出来**的东西就现场算（现在板子只判族标签 `'FAT'`，子类型由宿主机用算术反推）；② 出现 FAIL 时先怀疑判据 —— 这次两条都是判据错，而被测代码是**对的** |
| 58 | ★ **判据写成了"我以为的平台"——而它在另一个平台上会红** ★ | 同批(M4A-1.4):为了钉住"落地层把上游 `mutex_t` 当 16 字节存储用"这个前提,我在 `src/mutex.c` 里写了 `_Static_assert(sizeof(mutex_t) == 16u, …)`。这个数**只在 32 位目标上成立**(ARM:`state`4+`owner`4+`rcc`4+`rec`1+填充3=16);宿主单测把 `mutex.c` 编到 **x86_64** 上时 `tcb_t` 是 8 字节指针 ⇒ 结构体 24 字节 ⇒ **断言失败、`test_arm32_mutex.py` 当场红**。<BR>⇒ 改写成 `sizeof(void *) != 4u \|\| sizeof(mutex_t) == 16u`:宿主上第一项为真、断言自动通过,ARM 上第一项为假 ⇒ 真正去核那个 16。断言要说的本来就是"**在指针 4 字节的平台上**如何",写成无条件就等于把宿主也框了进来。<BR>★ **过程上的教训更值钱**:上一笔提交说明写了"全套 116 passed",而当时**只跑了新加的用例** —— 动了 **M4-11.1 已验证过的子系统**(mutex)却没跑全套。补跑后是 10 failed(其中 1 条就是这条回归),修完回到基线 9 failed / 116 passed。⇒ **碰已验证子系统必须跑全套**,不能只跑新写的那个文件 |
| 59 | ★★ **重复定义被 `-z muldefs` 静默放行 = "看起来能跑"；以及"上游写在拷贝上的清理"** ★★ | M4A-1.5 把上游 `dev.cpp` 加进图时发现:移植侧的 devfs **骨架**与上游 dev.cpp **都定义** `devfs_register`/`devfs_delete`(C 链接、同名)。本项目内核链接带 `-Wl,-z,muldefs`(源 OS 自己就带)⇒ **ld 静默挑一个**,谁生效取决于命令行顺序 —— 而两种实现的语义**并不相同**(骨架没有 `/dev` 节点)。<BR>★ 这一条与坑表 55 是**同一族**("同名/重复必须有唯一真相"),但更阴:55 是"定义了却链不上",这里是"链上了但不知道是哪一份"。⇒ 处置:显式二选一(选上游),骨架**删除**而不是留着。<BR>★ 同一批还抓到第二个:上游 `delete_device()` 把 `dev.path = NULL; dev.flag = 0; dev.vdiskid = 0;` 写在**局部拷贝**(`device_t dev = device_ctl[vdiskid];`)上 ⇒ 设备表里没清,删掉的设备变成"幽灵"( `get_device` 仍返非 NULL 且 `path` 已释放)。**判据(删除后必须取不到)一加就红** ⇒ 按上游**写出来的意图**修(同 D16 型),并把它钉成板上判据。<BR>★ 教训:上游那些"写了但没生效"的清理语句,是**意图的书面证据** —— 比"照字面抄"更该信;但改动必须**有判据**,否则就是替上游改行为还说不清 |

| 60 | ★★ **同一对端点的两个方向可以有"相反"的约束 —— 而它们只在真跑起来时才现形** ★★ | M4A-1.5 的 pty 配对验收第一版**挂住了**：日志停在 `pty: initialized` 之后，没有任何别的输出（也不是崩，是空转）。根因是两个默认 termios 行为叠加（`pty.cpp:60-62` 的 `c_oflag = OPOST\|ONLCR`、`c_lflag = ISIG\|ICANON\|ECHO`）：写主设备那条是**输入**路径，`ICANON` 下 `pts_data_available()`（`pty.cpp:117-129`）**只认行结束符**，缓冲区里没有 `'\n'`/VEOF/VEOL 就返回 0 ⇒ `pts_read` 一直空转；而写从设备那条是**输出**路径，`OPOST\|ONLCR` 会让 `pts_write()` 把 `'\n'` 改写成 `"\r\n"` ⇒ 载荷反而**不能**含 `'\n'`。<BR>★ 我是**特意**避开 `'\n'` 的（怕 ICRNL 改写把比对搅乱），结果正好踩在规范模式的放行条件上 —— **"为了稳妥而做的选择"本身可能就是那颗雷**。<BR>★ 还有一层顺序：默认 `ECHO` 开着，先做输入方向会把回显留在另一侧的缓冲里、挡住输出方向的读取 ⇒ 先 B 后 A。<BR>★ 教训：<B>端点语义不能只从"读/写"对称地去想</B>；行规约（canonical/ONLCR/ECHO）是**方向相关**的。判据要跑得起来，就得先把这几条默认值读出来（它们都在 `pty_termios_default()` 里），并把约束写成注释与断言（本笔两样都做了） |
...
>
> ⬜ ~~**M4A-1.2b 唯一还没做的验收项**~~ ⇒ ✅ **已完成（2026-09-19）**：
> `arm_vfs_bringup()` 把 `vfs_init()` + `tmpfs_setup()` 接进启动路径（顺序照源 OS
> `main.cpp:457`/`:558`），"**建 / 读 / 写 / 列目录**"四条判据 + 挂载见证（fsid 不同）
> + cwd + **反面控制**共十条全部上板通过：**110/0**、八组 A/B 全检出。
> ★ 结构上分两处：**调用在 C++ 落地层**（`upstream_api.cpp` 的七个 `arm_vfs_*`，
> 因为 `vfs.h` 整个文件没有 `extern "C"` ⇒ VFS 是 C++ 链接），
> **判据在移植侧 C**（`arch/arm32/src/vfs_check.c`），交界线由
> `tests/test_arm32_vfs.py` 钉住。
> ⇒ ~~**下一步：M4A-1.3（块设备 + `diskio`）与 1.4（FATFS，先接 RAM 盘）。**~~
> 
> ★★ **M4A-1.4 也已完成（2026-09-19）：板上 127/0。** 验收判据"**挂载 + 读写文件 +
> 与主机侧比对**"三条都上了板。★ 动手前的一个发现省掉了整个块驱动:
> **这个 OS 的块层就是 VFS** —— `diskio.cpp` 把 `disk_read()` 落成
> `vfs_read(node, buff, sector*512, count*512)`,没有独立的块设备 API ⇒
> **RAM 盘 = 一个 8MB 的 tmpfs 文件**,零新增块驱动代码,走的正是决策 3 说的那条
> "与真实块设备完全相同"的路径。
> 
> ★ 5 个上游 FATFS 文件(含 2MB 的 `ffunicode.cpp`)**零修改**编过,链接缺口正好
> **8 个符号**,全部是 **C++ 链接**(`_Z12mutex_createP5mutexb` 实测)⇒ 实现在 C++
> 落地层、**不能**写 `extern "C"` —— ★ 与 VFS 那组**正好相反**,规则由"符号由谁调"
> 决定。三个决策(理由见 README 的 M4A-1.4 一节):`FM_FAT` 而不是上游写死 FAT32 的
> 辅助函数(FAT32 要 ≥ ~34MB,堆正好 32MB);互斥锁把上游对象当**不透明存储**交给
> M4-11.1 验过的那把 mutex;`realtime_ns()` 是开机以来的纳秒数(Zynq 上没有 CMOS,记 D24)。
> 
> ★ **"与主机侧比对"是第三条独立证据**:板子把引导扇区原样打出来,宿主机用**自己的**
> 解析核,并**由算术反推子类型 = FAT12**(不看任何标签)。⚠ 这一步也修掉了**两个
> 我自己的判据 bug**(写死"必须是 FAT16"、以及把 `55 AA` 当小端比)—— 见坑表 57。
> 
> ⇒ **下一步:M4A-1.3(真实块设备 SD/arasan)与 M4A-1.5(`procfs`/`dev`/`pipe`/`pty`)。**
...
| 坑 54：shell 整段输出排他 | `e5e5aba` `6a4b91a` | `shell_test.py` 回到 **10/10**（修复前 9/10）|
| **仓库对外呈现：根 README 重写 + 两份对外文档** | 紧随其后的文档提交 | `README.md` 开头即 WIP/不可用声明（上游 README 逐字节备份到 `OLD_README.md`）；新增 `docs/ZYNQ7020_PORT_STATUS.md`（现状/改动/构建/测试/依赖关系）与 `docs/ZYNQ7020_INTEGRATION_PLAN.md`（合流路线/ABI·API 不变清单/`arch/arm32` 接入上层抽象/决策 D1–D6）|
...
##### ✅ 规划缺口已结案（2026-09-18 拍板，合流决策 D4）：**新增 M4A-4，且 M5 的验收原话要改**

原来的缺口：M4 那一行写着"用户态被推到 M4 之后，见 §4.6 的 M4A 系列"，
但 **§4.6 的 M4A 系列当时只有 1/2/3**，而 **M5 的验收是"挂载 `/system` 并跑 `shell.elf`"**。

**结论**：走"补一个 **M4A-4「用户态与 syscall 层」**"，放在 M4A-3 与 M5 之间。

★ 拍板时又查出**原分析漏掉的一件事**：M4A-4 与"跑 `shell.elf`"**不是同一件事的远近**，
所以 M5 的验收原话必须跟着改。

| 事实 | 出处（本次逐条核过） |
|---|---|
| **传输层与处理体真的可分开** | `kernel/syscall/syscall.cpp` 712 行（含**恰好 679** 处 `regs->r*`）vs `kernel/syscall/sys.cpp` **4,623 行**的处理体 |
| 私有号是硬阻塞 | `include/syscall/pxapi.h:25` `SXAH_SYSCALL_RETURN = 128956723895689201` ≈1.29e17 > 2^56，装不进 32 位 `r7`，**必须重编号**（用户态二进制本来就要全部重建 ⇒ 免费） |
| ELF 侧只缺一个 typedef 层 | `kernel/user/user.cpp:555-683` 只认 ELF64（`e_phentsize != sizeof(Elf64_Phdr)` 是事实上的 64 位闸门）；但 `include/elf.h` **已经有** `ELFCLASS32`、`EM_ARM 40`、`Elf32_Sym`、全套 `R_ARM_*` |
| 用户入口极简 | `user/xapi/arch/x86_64/crt0.S` 只有 **9 行** |
| ★ **坑在用户态那一侧，不在内核** ★ | 今天**没有能给 ARM 用的树内 libc**：`user/xapi/include/xposix/*` 只有声明，`__xposix_ret`/`__xposix_ptr` **全仓库只有声明、没有定义**，`xposix/sys/stat.h:48-63` 编码的是 **x86_64 的 `struct stat` 布局**；所有预置二进制都是 ELF64 `EM_X86_64` |

⇒ 于是 D4 实际是**两个问题，答案不同**：

1. **M4A-4 本身有界、可上板二值判定**：SVC 分发 + `switch_to_user` + ELF32 加载 +
   一个**手写的静态链接 32 位测试程序**（判据＝用户态 `write` 出字符串 + `exit` 返回码）。
   它**不需要** musl/busybox，因此不依赖任何一次交叉编译。
2. **M5 的原话"跑 `shell.elf`"即使做完 M4A-4 也做不到** —— 那要 ARM 用户态**加**重建
   busybox/musl，而计划原本就把"重建 busybox/musl"放在 **M7**。
   ⇒ **M5 的验收改成「M4A-4 的用户态程序 + 内核侧挂载 `/system` 并读文件」，
   "跑 `shell.elf`" 明确留给 M7。**

⚠ 原文那句"**M5/M6 的验收都依赖用户态能跑**"里，**M6 那一半存疑**：`netserver.sys` 是
**内核模块**（`kmod/netserver/`，入口 `dlmain`），M6 的直接依赖看起来是**模块加载
（合流决策 D3）**而不是用户态。这一点**尚未逐行核实**，所以本次只改了 M5 的验收，M6 保持原样。
...
| ~~**用户态没有归属的阶段**~~ | **已结案（2026-09-18，合流决策 D4）**：**新增 M4A-4**，放在 M4A-3 与 M5 之间；**同时改写 M5 的验收**（不在 M5 跑 `shell.elf` —— 那还要 ARM 用户态加重建 busybox/musl，是 M7 的活）。核实过程与全部证据见 §0.5.7 末尾 |
...
| **M4** | ~~用户态~~ → **已重新规划为"内存 / 栈 / 调度器"** | 本行是旧规划。现行范围见 §4.5（第一组 M4-1..M4-5 内存与栈、第二组 M4-6..M4-11 调度器）。**用户态不在 M4** —— 它是 **M4A-4**（2026-09-18 拍板，合流决策 D4）| ✅ 已完成（M4-1..M4-11；板级自检 **97/0** + 八组破坏性 A/B 全部检出）|
| **M5** | 存储 + rootfs | SD/SDIO 驱动（ADMA2 + 缓存维护，注册为 `device_t`）；FATFS 打通；镜像流程出 `BOOT.BIN`；**内核侧**挂载 `/system` 并读文件 + 跑 **M4A-4** 的用户态程序。★ 验收原话是"挂载 `/system` 并跑 `shell.elf`"，**2026-09-18 改写**：「跑 `shell.elf`」**移给 M7**（那要 ARM 用户态**加**重建 busybox/musl）。理由见 §0.5.7 末尾 | |
...
M4A-4 用户态与 syscall 层          <- 2026-09-18 新增(合流决策 D4);见 §0.5.7 末尾
```

★ **M4A-4 是本次拍板新加的**（原表只到 M4A-3，而 M5 的验收要用户态先存在）。
它**不依赖任何交叉编译**：判据是手写的静态链接 32 位测试程序，不是 `shell.elf`
（后者要重建 busybox/musl，留给 M7）。理由与全部证据见 §0.5.7 末尾。
...
| **M4A-1.2b** | **VFS 核心 + `tmpfs`** | 1.2a ✅ | **第一个完整切片**：建/读/写/列目录全在内存，不需要块设备。<BR>⚠ 动手前已实测上游 `driver/fs/**` 的耦合面，**结论是"搬 + 薄适配"而不是重写**：`tmpfs.cpp` **0** 耦合、FATFS `ff.cpp` **0**、`vfs.cpp` **31**（`get_current_task` 8 + PCB 字段 14 + `device_t` 9，集中在 cwd / fd 表 / tty / pid）、`procfs.cpp` **43**（进程信息 fs，天然要 PCB）。<BR>**进度：**<BR>✅ **1.2b-1 已完成（`5d9ebba`）**：`include/fs/vfs/vfs.h` 与 `include/fs/vfs/list.h` 的 `proto.hpp` 过度包含去掉（`list.h` 那条经 `mm/memory.h` → `efi/efi.h`，而 efi.h 用 `ms_abi`（MSVC/x86 调用约定），ARM 直接报 `'ms_abi' attribute directive ignored [-Werror=attributes]`）；`dnsfs/nmfs` 补上显式 `proto.hpp`（它们用 `snprintf`，而那个声明**就在 proto.hpp 里** —— 用预处理定位过）。结果：**ARM 的 C++ 能包含 `<fs/vfs/vfs.h>` 了**（探针 TU rc=0）。<BR>✅ **架构覆盖层已落地（`7582e3b`）**：`arch/arm32/include/upstream/cpu/lock.h`（上游那份是纯 x86 汇编，9 个 `driver/fs` 文件包含它）+ 三处平台中立修复（`include/stdint.h` 的 `int8_t` 在 ARM 上本就是错的无符号；`NULL` 守卫；`lock_queue.h` 的**引号包含绕过 `-I` 顺序**导致两份 `spin_t` 并存）。<BR>⬜ **剩余（已逐条定位）**：<BR> ① ~~`<cpu/lock.h>` 的 x86 汇编~~ ✅ 覆盖层已解决；<BR> ② ~~指示符顺序~~ ✅ 已修（`dev.cpp` / `pipefs.cpp`，只重排、值不变）；<BR> ③ ~~缺调度器 API~~ ✅ 覆盖层补回了 `<task/scheduler.h>`（实测那个头在 ARM 上 rc=0）；<BR> ④ ~~汇编器 `strex r3,r2,[r3]`~~ ✅ **独立成一笔真 bug 修复**（`a552759`）：`arch_strex`/`arch_ldrex` 缺 early-clobber，寄存器分配是上下文相关的 ⇒ 一直是潜伏的；上板 `smp_lock_counter=40000` / `violations=0` 验证；<BR> ⑤ ~~partition/procfs/pty/socketfs 的错误提取~~ ✅ 已提取：`partition.cpp` 的 `pause` 是它自己手写的汇编（改用 `cpu_relax()`）；`procfs.cpp` 是**真正的 x86 专属文件**。<BR>✅ **`fb23303`**：`dev.cpp` / `pipefs.cpp` / `vfs.cpp` / `pty.cpp` / `socketfs.cpp` / `tmpfs.cpp` / `unixsock.cpp` / `dnsfs.cpp` / `nmfs.cpp` **全部编过**（`-c` 零错误）—— 含 **VFS 本体 `vfs.cpp`（1433 行）**。ARM 侧扫描 **16/17**，唯一编不过的是 `procfs.cpp`（`/proc/cpuinfo` 的 **CPUID 特性名表** `[0]="pni",[25]="sse",[28]="avx"`，GCC 的 C++ 不支持数组指定初始化器 ⇒ 它就是 x86 专属，不是"还差一点"）。<BR>⬜ **下一步就是链接**：把 `vfs.cpp` + `tmpfs.cpp` 加进 `ARM32_UPSTREAM_CXX` 后，编译全过、**链接缺 18 个符号**（已列在提交 `fb23303` 里）。<BR>✅ **`a8de6d2`：18 → 11**。解决的 7 个：`queue_get`/`queue_dequeue`/`queue_destroy`（★ **纯复用** `kernel/lock_queue.cpp`，实测 ARM 零错误）、`scheduler_yield`/`scheduler_sleep_ns`/`scheduler_wake_task`（★ 移植侧 `arch/sched.h` **早就有同名同形的三个函数**，只差前缀）、`disable_intr`/`enable_intr`（`arch_irq_*`）。载体是新增的落地层 `arch/arm32/src/upstream_api.cpp` —— **移植侧唯一的 C++ 文件**，理由：上游 `task/scheduler.h:10-12` 的声明没有 `extern "C"`，C 实现给不出对应符号（实测报 `undefined reference to 'scheduler_yield()'`）。它**只转发不写逻辑**。<BR>★ 由它打出来一条**通用规则**（已写进头部注释）：**移植侧的头文件只要会被 C++ 翻译单元看到，就必须声明 C 链接** —— `arch/sched.h`（实测）、上游 `include/device.h`（`regist_device(char const*, _device)` 链不上）、`arch/device.h`、`krlibc.h` 都已照办；对照 `arch_irq_disable()` 是 `static inline`，不产生符号所以没这个问题。<BR>⬜ **剩余 11 个**（实测）：`get_current_task` / `kernel_group` / `get_current_directory` → **进程层最小切片**；`sprintf` / `write_serial_fmt` → 格式化器（上游在 x86 串口驱动 `driver/serial/serial_port.cpp`，ARM 编不了；移植侧 `console.c` 那个只往串口写、不往缓冲区写）；`get_random_bytes` → rng（上游 `kernel/rng.cpp` 用 x86 指令）；`free_frames` / `page_map_range_to_random` → **x86 页层的 mmap 路径**（ARM 的 vmap/page 模型不同，要单独想清楚）；`get_keyboard_input` / `p_xapi_output_kernel` / `pathacat` → 逐个查。<BR>⚠ 它们**没有**被加进构建图 —— 加了内核会链不上，而拿空壳假装兼容正是本计划 §4.4 警告的事。清单在 `tools/gen_ninja.py` 的 `ARM32_UPSTREAM_CXX` 旁边也写了一份。<BR>✅ **★★ 2026-09-19：缺口清零（11 → 2 → 0），`vfs.cpp` + `tmpfs.cpp` 已永久进构建图 ★★**。<BR>① **11 → 2**（落地层补齐，与 ② 同一笔提交）：`get_current_task`/`kernel_group`/`get_current_directory` = **进程层最小切片**（一个静态 `pcb_t` + `extern pcb_t kernel_group`，全部内核线程共用它 —— 正是源 OS `kernel/task/pcb.cpp:2726` 的模型）；`get_random_bytes` = xorshift32，种子取 333MHz 全局定时器；`free_frames` **不需要实现** —— 真因是上游 `include/proto.hpp` 里 `free_frame()` **没有 `inline`**（头文件里的普通函数定义 ⇒ 每个包含它的 TU 都生成符号、并且拖着 `free_frames`，哪怕一次都没调用过；`nm -u tmpfs.o` 实证）。加 `inline` 后引用消失（平台中立修复，与 `fb23303` 那批同类）。<BR>② **2 → 0**：`sprintf` / `write_serial_fmt` —— 把 console.c 原来"只往串口写"的格式化循环抽成**带输出汇（sink）的核心** `format_core`，三个入口共用同一个核心（`console_printf` 走控制台汇、`console_vprintf`/`console_vsprintf` 是 v 形式；换行转换从循环挪进汇，于是 `%c` 的 `'\n'` 也被补 CR）。<BR>★ **两次都只有工具能看出来的东西**（源码上完全看不出，已各进坑表一条）：<BR>  (a) **符号形状**：`include/proto.hpp:38,42` 把这两条声明写在 `extern "C"` **之外** ⇒ 上游调用点要的是**名字修饰过**的 `_Z7sprintfPcPKcz` / `_Z16write_serial_fmtPKcz`（`arm-none-eabi-nm vfs.o` 实证）。第一版在 C 里定义未修饰的 `sprintf`，链接器照样报 `undefined reference to 'sprintf(char*, char const*, ...)'` —— **同名、不同符号**。⇒ 定义移到 C++ 落地层转发（实现仍只有一份在 console.c）。<BR>  (b) **`-Wl,-z,muldefs`**：`include/fs/vfs/list.h` 在头里**直接定义**了 `list_delete` 一族（非 `inline`、非 `static`）。上游 x86 侧只有 `vfs.cpp` 一个 TU 包含它，所以从没暴露；ARM 侧 `vfs.cpp` + 落地层两个 TU ⇒ multiple definition。**源 OS 自己的内核链接就带 `-z muldefs`** ⇒ 与之对齐，而不是去改公共 ABI 头。<BR>★ **宿主单测在这一次抓到一个会打到板子上的真 bug**：`take_unsigned`/`take_signed` 收 `va_list` **传值** ⇒ 子函数里推进的游标**在返回时丢掉**，第二次转换起全部读同一个槽位（`console_printf("%d|%s", 42, "ok")` **访问违例**；`"%llx|%d"` 打出 `1432778632` = 那个 64 位量的低半部分）。**ARM EABI 的 `va_list` 是结构体、Windows x64 是 `char *`，两种 ABI 下传值都丢** ⇒ 这个写法在**目标板**上同样会打出错值。改成 `va_list *` 后 8 组用例全对。<BR>**验收状态**：`ninja -f build-arm.ninja arm32` = **39/39、0 个未定义符号**，ELF 里 `_Z8vfs_initv` / `_Z9vfs_mountPKcP8vfs_node` / `tmpfs_*` 全在；板上 **100/0**、八组破坏性 A/B 全部检出（`verify_board.py --load --seconds 75` **退出码 0**）。宿主 `tests/test_arm32_console.py`：**7 条 + 10 子判据**（长度修饰符 / 精度 / `*` 宽度 / `%#x` 前缀次序 / `%n` 空操作 / 空指针 / 三个入口的换行差异 + 4 条契约），全套 99 passed（9 条既有失败与本次无关）。<BR>✅ **★★ 1.2b 的验收项已完成（2026-09-19）：上游 VFS 在板上真的跑起来了 ★★**。<BR>`vfs_init()` + `tmpfs_setup()` 已接进启动路径（`arm_vfs_bringup()`：进程上下文 → `vfs_init()` → `tmpfs_setup()` → 根目录填进 cwd，顺序照源 OS `kernel/main.cpp:457` 与 `:558`）。板上 **110 passed / 0 failed**、八组 A/B 全检出（退出码 0），十条新判据：<BR>`vfs_bringup=1` / `vfs_tmpfs_mounted=1` / `vfs_create=1` / **`vfs_write=32`** / **`vfs_read=32`** / **`vfs_roundtrip=1`** / `vfs_list_delta=1`（`vfs_list_before=0`，之后 ≥ 2）/ `vfs_cwd_root=1` / `vfs_negative_ok=1`。<BR>★ **判据分三层,而且第三层不是装饰**（见 `arch/vfs_check.h` 文件头）：① 32 字节模式**写进去读回来逐字节相同**；② **`/tmp` 的 `fsid` 必须不同于 `/`** —— 那才是"挂载真的发生"的证据（`tmpfs.cpp:111` 的 `node->fsid = tmpfs_id`；没挂上就只是根 fs 的普通目录）；③ **反面控制：在根 fs 上做同样的往返必须不成立**。第三层的必要性来自一个**上游的实现细节**：给未挂载节点的回调是 `static void empty_func() {}` —— **返回 void 却被塞进 `size_t (*)(...)` 的槽**，于是"调用成功"本身**不携带信息**（返回值未定义）。只看前两层，"建文件成功"完全可能是假通过（坑 43 的同型）。<BR>★ 一处**结构决定**:`include/fs/vfs/vfs.h` **整个文件没有 `extern "C"`** ⇒ VFS 的公开函数全是 C++ 链接（`_Z8vfs_initv` 实测）⇒ 移植侧 C 调不了。于是分成"**调用在 C++ 落地层、判据在移植侧 C**"（`arch/arm32/src/vfs_check.c`），落地层的七个 `arm_vfs_*` 全是 `extern "C"` 的纯转发。由 `tests/test_arm32_vfs.py`（**9 条 + 27 子判据**）钉住这条交界线：声明/定义同形、必须带 `extern "C"`、上游函数必须真在上游头里有声明、`TMPFS_REGISTER_ID` 与 `tmpfs_mount()` 的判据必须同一个数、以及"`vfs.h` 仍然没有 `extern "C"`"（这条是整组断言的前提，上游变了它就该失败）。<BR>⬜ **1.2b 之后**：M4A-1.3（块设备 + `diskio`）与 1.4（FATFS，先接 RAM 盘） |
| **M4A-1.3** | 块设备（SD/arasan）+ `diskio` | 独立，可并行 | 读写扇区 + 写回读。<BR>⚠ **需要插卡才能验**：驱动可以写、可以编、可以链接，但"读写扇区 + 写回读"这条判据必须有一张真的 SD 卡在板上。⇒ 走到这一步时**弹窗提醒**(`tmp-test/notify_user.ps1`)。<BR>★ **M4A-3/B5 侦察已做（2026-09-19，见 README 的 M4A-3/B5 一节）**：上游 `driver/device.cpp` 能干净编过，但真缺口 **11 个符号**（8 个是 x86 页层 + 进程层 + 分区层 + `mutex_trylock`），而那 8 个是**架构差异**（DMA bounce / direct-span 只在 HHDM 下成立）⇒ **决定另写一份端口原生块层**（接口与偏移算术照上游，ARM 侧目的地址恒为内核地址 ⇒ 不需要 bounce）。 |<BR>★ **M4A-1.3 侦察已做(2026-09-19)**:控制器在描述表里就有 —— `sdhci0` / `compatible = "arasan,sdhci-8.9a"` / `reg_base = 0xE0100000` / `arasan,sdio-clk-freq-hz = 100000000`(`board_devices.c:33-175`),另有 `PLAT_SD0_BASE 0xE0100000`、`PLAT_IRQ_SD0 56`(`platform.h:74/171`)。驱动模型照 `axi_gpio.c`/`uart_ps.c` 的写法(移植侧 C + 描述表取基址)。<BR>★ 工作面**分成两半**,而这两半的判据完全不同:**① 不依赖卡**:软复位、Capabilities 读回、SDCLK 编程(从 100MHz 分频)、电源/中断寄存器写回读 —— 这些**今天就能上板验**;**② 依赖卡**:CMD0/CMD8/ACMD41 初始化 + CMD17/24 的扇区读写往返 —— **必须插卡**,而且那一步才是 "读写扇区 + 写回读" 这条验收。<BR>⚠ 判据要**先做①再做②**:①失败时不该去怀疑卡,②失败时也要能分清"控制器没配好"还是"卡没响应"。
| ~~**M4A-1.4**~~ | ~~**FATFS**~~ | M4A-1.2 ✅ | ✅ **已完成（2026-09-19）：板上 127/0、八组 A/B 全检出**。验收判据"**挂载 + 读写文件 + 与主机侧比对**"三条都上了板。<BR>★ **关键发现:这个 OS 的块层就是 VFS** —— `diskio.cpp` 把 `disk_read()` 落成 `vfs_read(node, buff, sector*512, count*512)`,**没有独立的块设备 API** ⇒ **RAM 盘 = 一个 8MB 的 tmpfs 文件**,零新增块驱动代码,而且走的正是**决策 3** 说的"与真实块设备完全相同"的那条 `diskio` 路径(M4A-1.3 的真实块设备接上后,`diskio` 一行都不用改)。<BR>★ 5 个上游文件(含 2MB 的 `ffunicode.cpp`)全部**零修改**编过,链接缺口正好 **8 个符号**:`mutex_create/lock/unlock`、`mktime`、`realtime_ns`、`ahci_is_qemu_environment`(×2)、`alloc_frames`、`phys_to_virt`。**全部是 C++ 链接**(`_Z12mutex_createP5mutexb` 实测)⇒ 实现在 C++ 落地层、**不能**写 `extern "C"` —— ★ **与 VFS 那组正好相反**(那边要求 `extern "C"`,因为调用方是移植侧 C);规则由"符号由谁调"决定。<BR>三条决策(各有完整理由,见 README 的 M4A-1.4 一节):① **`FM_FAT` 而不是上游写死 FAT32 的 `fatfs_format_node()`** —— FAT32 要 ≥ ~34MB,内核堆**正好 32MB**;(另:`alloc_number()` 是 `fatfs.cpp` 的 `static`,落地层拿不到)② **互斥锁**把上游 `mutex_t` 对象当**不透明存储**(移植侧只占前 16 字节)交给 M4-11.1 验过的那把 mutex,移植侧加三个 `arm_mutex_*` **换名字**入口(同名 + 两种链接在 C++ 里非法),两侧各一条静态断言钉住 16 字节;③ **`realtime_ns()` 是开机以来的纳秒数**(Zynq 上没有 CMOS —— 那份实现不是"没搬"而是"搬来也没硬件",记 D24)。<BR>★ **"与主机侧比对"是第三条独立证据**:板子把引导扇区原样打一行 `FATBS:<1024 hex>`,宿主机用**自己的**解析核(`tmp-test/verify_fat_bootsector.py`),并由"FAT 占 7 扇区 / 2043 簇"**用算术反推子类型 = FAT12**(不看任何标签)。<BR>⬜ **本步之外**:M4A-1.3 的真实块设备(SD/arasan)仍未开始 —— 今天验的是"卷是内存文件"这条路径 |
| **M4A-1.5** | `procfs` / `dev` / `pipe` / `pty` | M4A-1.2 ✅ | ⬜ **进行中**。✅ **`dev` + `pipe` 已进图（2026-09-19，板上 127/0）**：上游 `driver/fs/vfs/dev.cpp`（真 devfs）+ `pipefs.cpp` 零修改编过，链接缺口只有 4 个符号（3 个是"同名不同符号"那一类，坑表 55）。<BR>★ **devfs 骨架退场**（M4A-1.1b 那条退化项结案）：骨架与上游 dev.cpp **都定义** `devfs_register`/`devfs_delete`（C 链接），而内核链接带 `-z muldefs` ⇒ ld **静默挑一个**。必须显式二选一；选上游 = 源 OS 优先。<BR>★ **换掉之后 A/B 更强**：不再需要移植侧造的开关，直接用上游 `devfs_register()` 的第一句（`if (devfs_root == NULL) return EOK;`，dev.cpp:345）—— 挂载前注册的**找不到**、挂载后注册的**找得到**、注销后**又找不到**，板上 `pre-mount=1 post-mount=1 after-delete=1`。<BR>★★ **顺带抓到一个"幽灵设备"**：上游 `delete_device()` 把 `dev.path = NULL; dev.flag = 0; dev.vdiskid = 0;` 写在**局部拷贝**上（`device_t dev = device_ctl[vdiskid];`）⇒ 删掉之后 `get_device(id)` 仍返非 NULL（`path` 还指着已释放内存）、`have_vdisk(id)` 仍为真。按**上游写出来的意图**修（同 D16 型），并把它变成板上判据。<BR>✅ **`pipe` 的读写往返自检已补（板上 136/0）**：九条判据，含两个**独立观测点**（`pipe_fill_after=32` / `pipe_fill_drain=0` —— 管道里**现在**有多少字节，与"它说读写了多少"是两件事；只看后者，一个原样返回入参的空壳也能全绿）与短读语义（写 8、按 32 读 ⇒ 8，证返回量由**存量**决定）。★ 并立下一条**机器可判的安全约束**：启动自检**不许读空管道**（`pipefs_read` 会 `pipe_wait_on` 阻塞 ⇒ 当场挂住且没有任何日志），由宿主测试的顺序检查钉住。<BR>✅ **`pty` 已进图并起搏**（板上 138/0）：`pty.cpp` 的链接缺口只有 **1 个符号** —— `snprintf`，上游那个**有边界**的格式化函数（`serial_port.cpp:761`，声明在 `proto.hpp:31`、**不在** `extern "C"` 里 ⇒ 要修饰名 `_Z8snprintfPcjPKcz`）。移植侧补了有边界的缓冲区汇 `console_vsnprintf`（语义照上游、也是 C99：`size==0` 直接返回 0 且不碰 buf；最多存 `size-1` 个字符；★ 返回值是**本该写多长**，截断时大于实际长度 —— `pty.cpp:164` 拿它拼 `/dev/pts/<id>`）。板上判据 `pty_init=1` / `pty_ptmx_node=1`。<BR>✅ **pty 配对往返也已完成（板上 148/0）**：`vfs_open("/dev/ptmx")` 拿主设备 → **`TIOCGPTN` 问出**从设备号（不是假定 0；`ptmx_open` 每条 pair 都从共享的 `pty_id_alloc()` 取号）→ `TIOCSPTLCK(0)` 解锁 → 开 `/dev/pts/<n>` → **两个方向各一次真实往返**（A 写主读从、B 写从读主，走的是两块不同缓冲、两端两个不同节点）。<BR>★★ **上板才看得见的一道坎**：两个方向的载荷约束**相反** —— A 是输入路径，默认 `ICANON` ⇒ `pts_data_available()` 只认行结束符 ⇒ 载荷**必须以 `'\n'` 结尾**；B 是输出路径，默认 `OPOST|ONLCR` ⇒ `pts_write()` 会把 `'\n'` 改写成 `"\r\n"` ⇒ 载荷**必须避开 `'\n'`/`'\r'`**。第一版避开了 `'\n'`，结果 A 的 `pts_read` 空转、日志停在 `pty: initialized` 之后。★ 另外默认 `ECHO` 开着 ⇒ **先 B 后 A**，免得回显挡住 B 的读取（这条也写成断言）。<BR>⚠ 与 pipe 同一条安全约束：**不要读空的那一端**（会空转挂住），由宿主测试的顺序检查钉住（pipe 与 pty 各一条）。<BR>✅ **`procfs` 已作出决定:不搬,并记进构建图**。实测证据:`procfs.cpp:595` 的 **CPUID 位名表**(`[0]="fpu", … [28]="ht"`)是稀疏数组指定初始化器,GCC 的 C++ 前端报 'sorry, unimplemented: non-trivial designated initializers not supported'(上游用 clang 当扩展收下)。★ 就算语法能过**也不该搬**:那些位是 x86 的 EDX/ECX,ARM 的 `/proc/cpuinfo` 必须由 MIDR/MPIDR/CTR/CCSIDR 拼 ⇒ 这是"要另写一份 ARM 的"。⇒ 记成**显式排除**(`tools/gen_ninja.py` 的 `ARM32_UPSTREAM_EXCLUDED`,带"为什么"与"什么能解开它"),由 `tests/test_arm32_upstream_excluded.py`(4 条 + 12 子判据)钉住三条不变量:每条有实质理由、理由可核(行号/符号/阶段号)、**同一路径不能同时在两张表里**。解开条件:有东西真的读 `/proc`(现实里是 M7 的用户态)。<BR>⚠ 同表里还记了另外三条**同为"决定不搬"**的:x86 串口驱动(`serial_port.cpp`,8250 端口 vs Zynq 的 MMIO)、PC 的 CMOS 时钟(`rtc.cpp`,Zynq 没有该器件)、块层(`driver/device.cpp`,依赖分区层与 x86 帧分配器,属 M4A-3/B5) |
...
**决策 4（2026-09-18，合流决策 D2）：内存分配器保留 ARM 那份，只补一层上游形状的接口。**
M4A-1 会拉进来的 19 个文件里，堆 API 只有 **4 个符号**、**257 处调用**
（`free` 187 / `malloc` 55 / `calloc` 10 / `realloc` 5；`aligned_alloc` 与 `usable_size`
在这个集合里 **0 处**），外加 `PAGE_SIZE` 51、`MIN` 19、`MAX` 4、`PADDING_UP` 3 ——
而 ARM 侧今天**没有这些名字**（只有 `PALLOC_PAGE_SIZE`/`VMAP_PAGE_SIZE`）。
⇒ 一个 ~40 行的兼容头 + 一个把 `heap_alloc(&g_kernel_heap, …)` 包成
`malloc/free/calloc/realloc` 的 shim 即可，
不必搬 `kernel/memory/**`（那是计划 §2.3 自称"**单项风险最高**"的工作包：`page.cpp` 871 行
+ HHDM + UEFI 内存图 + x86 4 级页表，共 2,196 行）。
...
> ⚠ **`docs/DRIVER_GUIDE.md` 没有进版本库**（它和 `docs/ARCHITECTURE_MAP.md` 是本机
> 生成的参考文档，一直是未跟踪状态）。本节引用它的行号来自本机那份；对外发布时
> 这几个引用会**指不到文件**。结论（"导出面只有约 76 个符号、缺 EOI/中断注册/锁原语"）
> 本身可以用 `grep -n EXPORT_SYMBOL kernel/**` 一类命令独立复核。
