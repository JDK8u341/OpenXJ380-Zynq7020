...
**M0–M4-9.5 全部完成并板上验证；M4-8.4、D7 已结案；M4-8.5（无饥饿判据）也已完成。
当前板级自检 72 passed / 0 failed。**

**五个**破坏性 A/B 全部成立：搬帧（`a=0 b=0`）、VFP 现场（`bad=(7914,0)`）、
扫描唤醒（`ctl_wakes 0->0`）、不换栈（`b_sp` 落在启动栈）、
**无饥饿（`STARVATION: ... missed=4/4`，`adv=1/4`）**。

**下一步 = M4-10（SMP 调度，照源 OS）**，范围已核实并写进 §0.5.7
（★ 源 OS 没有周期性负载均衡/迁移，所以**不发明均衡器**）。
之后是 M4-11 → M4A 系列。
...
### 0.5.7 ★ 续接点：**下一步 = M4-10（SMP 调度，照源 OS）**（M4-8.5 已完成并板上验证）★

> **这一节是压缩上下文后的第一读物。** 它写清了：M4-9/M4-8.5 各自落成了什么样、
> **下一步 M4-10 要怎么做的全部细节**、之后 M4-11 与三条 API 兼容性分别在哪个阶段、
> 以及一个需要拍板的规划缺口。

**M4-9（抢占）、M4-9.5（VFP 现场）、M4-8.4（周期状态行）、D7（调用边界对齐）、
M4-8.5（无饥饿判据）都已完成并板上验证。** 板级自检 **72 passed / 0 failed**。

#### 已完成的后五步（摘要；每步的细节在各自的提交说明里）

| 步 | 提交 | 落成了什么样 |
|---|---|---|
| **M4-9 搬帧** | `c45431b` + `79947bc` | 帧路径 + **`svc` 陷阱式让出**（源 OS 的 `scheduler_yield` 就是 `scheduler_ticks=TIME_SLICE; int $32`）。`sched_ctx_switchable()` 四条绊线。破坏性 A/B `g_reloc_skip` ⇒ 两个线程**一次都跑不起来** |
| **M4-9.5 VFP 现场** | `53da560` | 补 M4-6 **写明却从没实现**的账。★ 存/取**卡在 C 调用链的两端**，因为 GCC 在本内核里已用 `vldr d16/vstr d16` 做 64 位清零 ⇒ 判据里不能含"编译器不会用 VFP"这个前提 |
| **M4-8.4 周期状态行** | `4e98d1d` | 第一个**会睡会醒**的真实负载。★ 顺带补上 **D4**：队列改 **FIFO**、选取改**全表扫描**、扫描里**带唤醒** ⇒ `sched_tick` 里那套队列维护**整段消失** |
| **D7 调用边界对齐** | `d03a639` | `bic sp, sp, #7` **一条指令**；对齐的是**调 C 的边界**，不是帧。硬自检 `c_handler_sp_aligned` 必须 0 |
| **M4-8.5 无饥饿判据** | `4d8aa2e` | 三层判据（宿主的策略穷尽 / 板上的机制二值 / 破坏性 A/B），外加一个**常驻饥饿监视器**。见下 |
...
#### 下一步：**M4-10 照源 OS**（用户已拍板，范围已核实）

★ **源 OS 没有周期性负载均衡/迁移** —— 全树搜 `balance|migrat|load_avg|steal` **零命中**。
所以 M4-10 **不发明均衡器**，范围是：
...
| 10.6 | VFP 切换去掉 CPU0 护栏（`arch_vfp_switch` 里那条）| 与 10.2 同步 |
| A/B | 选核改成"永远给 CPU0" ⇒ CPU1 的 `switched[1]` 恒为 0 而 CPU0 队列积压 | 可读数 |

⚠ 现在 `sched_tick` 里有 `if (pc->cpu_id != 0) return frame;` —— 那是临时护栏
（`g_runq[1]` 与 CPU1 的 idle 都还没建，放 CPU1 过去会**两个核同时往一个上下文里塞现场**）。

⚠ **M4-10 之前不要再往 `sched_tick` 里加东西**：它现在同时管计费、可调度性、
帧搬迁三件事（队列成员关系已在 M4-8.4 对齐源 OS 后消失），而每核化会把这些全部重写一遍。

#### 之后：M4-11 → M4A-1 → M4A-2 → M4A-3(B5 起才是驱动 API 兼容)
...
| **M4-10 里的那条时序敏感点** | A/B 窗口内 `invalid` 的时序**对代码改动敏感**（早先 `0->0`，M4-9.5 之后 `0->53`）。数值现在与推演一致，但**敏感点没定位**。每核化会重写这一片 ⇒ 交给 M4-10 |
...
| 就绪队列无锁（D12）| 单核、只有 CPU0 碰它。M4-10 再定每核一把还是无锁 |
| `spin_t` 的宿主编译器 `#include <stdint.h>` | 无 |
| `arch/arm32` 不在 `ninja format` 的范围内 | 刻意；见 README（用 clang-format 会打散注释对齐）|
| **心跳区 32 槽已满** | 再加诊断量就没地方了，而 M4-10 还要**按核**读数。扩容 OCM 布局还是删旧槽，**要拍板**（M4-8.5 的监视器因此只走串口，没占槽）|
...
| **M4-10** | **SMP 调度**：per-CPU 队列 + **创建时选核** + 亲和 | M4-8.5 | ⬜ 范围已核实并拍板（**照源 OS，不做周期性迁移**，见 §0.5.7）|
