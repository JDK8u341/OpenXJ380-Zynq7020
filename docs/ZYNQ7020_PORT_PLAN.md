...
**M0–M4-9 全部完成并板上验证（含抢占）；当前板级自检 62 passed / 0 failed。**

M4-9 分两步：协议与决策层（`d8c49a2`），以及**搬帧**（`c45431b` + `79947bc`）。
"两个绝不让出的线程必须真的交错执行"这条验收判据**已在板上成立**
（`a=14073 b=14047 saw=(1,1) sp_bad=(0,0)`，30 次切换、27 次真抢占、0 次拒绝），
并且有一个破坏性 A/B 证明搬帧承重（关掉搬帧 ⇒ 同样两个线程**一次都跑不起来**，
计数 `0/0`）。
...
### 0.5.7 ★ 续接点：M4-9 **已完成**；下一步是 M4-8.4 / M4-10 / M4-11 ★
...
1. **M4-8.4：把 1Hz 状态行改成内核线程**。它是第一个"真实负载"，
   会睡会醒 ⇒ 正好验 `sched_apply_wakeup_credit()` 的**顺序**（不是比例）。
2. **M4-8.5 无饥饿判据** —— 现在抢占是真的了，才第一次可观测。
3. **M4-10：每核队列 + 负载均衡 + 亲和性**。
   ⚠ 现在 `sched_tick` 里有 `if (pc->cpu_id != 0) return frame;` —— 那是临时护栏
   （`g_runq[1]` 与 idle 都还没建，放 CPU1 过去会两个核同时往一个上下文里塞现场）。
   M4-10 要拆掉它，并顺带查清 §0.5.8 里那条"推演与实测不符"。
4. **M4-11：可睡眠互斥 + 内核线程 API**，把跑马灯/状态行/shell/心跳搬进线程。
5. 之后才是 **M4A-1 fs → M4A-2 GIC/UART/timer 拆分 → M4A-3 B 系列**。

⚠ **M4-10 之前不要再往 `sched_tick` 里加东西**：它现在同时管计费、
可调度性、队列成员关系、帧搬迁四件事，而每核化会把这四件全部重写一遍。
...
| 项 | 状态 |
|---|---|
| ~~**VFP 上下文根本没保存/恢复**~~ | **已结案（M4-9.5，`53da560`）**：板上 **65/0**，正例 `bad=(0,0) rmode_bad=(0,0)`、对照组 `bad=(8183,0) rmode_bad=(8184,0)` ⇒ 破坏性 A/B 成立。★ 关键决定是**放在 C 调用链之外**：GCC 在本内核里已用 `vldr d16/vstr d16` 做 64 位清零，而 `sched_tick` 里 `pc->scheduler_ticks = 0u` 正是"64 位存零" ⇒ 判据里不能含"编译器不会用 VFP"这个前提。见 README 的「VFP 现场：补账」|
| **A/B 窗口内 `invalid` 那条留痕** | 先前"推演与实测不符（`0->0`）"在 M4-9.5 之后变成 **`0->53`**，与推演一致（220ms ÷ 4 tick ≈ 55）。先前那次为什么是 0 **没有查明**，现象不再复现。已按实际改写注释，并记下"这条路径的时序对代码改动敏感、敏感点未定位"⇒ 交给 M4-10 |
| ~~★ **异常帧可能不是 8 字节对齐** ★~~ | **已结案（D7，`arch` 侧已修）**：实测一轮启动 **1688 次**（早先"20~28 次"是在启动早期取的，**被低估了**；负载加重后暴露得更充分）。<br>★ **修法与计划里猜的不一样**：计划写的是"帧 16→18 字、把原始 SP 存进帧"。**那是想岔了** —— 问题不在"帧落在哪"，而在**调 C 的那个边界**。`EXC_FRAME_LEAVE` 靠 `帧基址 + 64 == S` 还原被中断的 SP，**帧不能挪**；而 C 函数拿到的是 **r0 里的帧指针**（不是 SP），所以只要在 `bl` 之前对齐 SP、回来用 `mov sp, r0` 恢复即可 —— `bic sp, sp, #7` **一条指令**，不动帧、不动任何偏移与断言。<br>判据：新增硬自检 `c_handler_sp_aligned`（必须 0），由 `c_irq_handler`/`c_svc_handler` 在入口读自己的 SP 直接量 —— 实测 **0**（修复前它必然等于 1688）。<br>⚠ 顺带改正三处**错误因果**注释：帧大小是不是 8 的倍数**决定不了** SP 对齐（位置才决定）；M4-2 那次 Undefined 是 FPU 没使能，与对齐无关 |
| ~~`runtime_ticks` 从不累加~~ | 已清（M4-9.5）：照源 OS `scheduler.cpp:398` 每 tick 加一 |
| ~~`sched_tick_account()` / `SCHED_MAX_SWITCHES_TRACKED` 死物~~ | 已删（M4-9.5）|
...
| **M4-8** | 就绪队列 + 调度策略 + idle（WFI）| M4-7 | ✅ 已完成（`abde043`，54/0）|
| **M4-9** | **tick 抢占**（复用 AM3-5 的每核 1kHz）| M4-8 | ✅ **已完成**（`c45431b` + `79947bc`，**62/0**，含破坏性 A/B）；落地形状与计划的差别见 §0.5.7 |
| **M4-10** | **SMP 调度**：per-CPU 队列 + 负载均衡 + 亲和 | M4-9 | ⬜ 下一步（要拆掉 `sched_tick` 里的 CPU0 护栏，顺带查 §0.5.8 那条）|
