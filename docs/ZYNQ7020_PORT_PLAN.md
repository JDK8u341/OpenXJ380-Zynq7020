...
| ★ **异常帧可能不是 8 字节对齐** ★ | 实测一轮 **20~28 次**。SP 合法地可以是 4 mod 8（libgcc 的 Thumb `__udivmoddi4`，`push {r4,r5,lr}` = 12 字节），于是 `bl` 进 C 处理函数时 SP 也不对齐，严格说违反 AAPCS。**本板实测扛得住**，但那是"没观察到问题"。根治要把帧从 16 字加宽到 18 字、并把原始 SP 存进帧里 —— 用 `irq_frame_unaligned8` 量出的规模决定值不值得做 |
