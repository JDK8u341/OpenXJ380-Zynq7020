...
**M0–M4-10 全部完成并板上验证。当前板级自检 81 passed / 0 failed。**

**六个**破坏性 A/B 全部成立：搬帧（`a=0 b=0`）、VFP 现场（`bad=(0,7958)`）、
扫描唤醒（`ctl_wakes 0->0`）、无饥饿（`STARVATION: ... missed=4/4`）、
**选核（`pick_cpu0=1` ⇒ `cpu1 switched=+0`）**、不换栈（`b_sp` 落在启动栈）。

**两个核都在跑线程**：`cpu1 switched=+16 preempted=+15 idle_harvested=1 back=1`，
`cpu0 switched=+13 preempted=+7`，新线程都落在"造它之前更短的那个队列"上
（`place_bad=0`）。D6（CPU0 护栏）、D12（队列无锁）、D15（vruntime 补偿）一起结案。

**下一步 = M4-11（可睡眠原语 + 线程退出 API，结掉 D13/D14）**。
之后是 M4A 系列（fs → 设备驱动 → 驱动 API 兼容）。

⚠ 还有**一个待拍板的规划缺口**：用户态没有归属的阶段（M4A 系列只有 1/2/3，
而 M5 的验收要跑 `shell.elf`）。见 §0.5.7 末尾。
...
### 0.5.7 ★ 续接点：**下一步 = M4-11（可睡眠原语 + 线程退出，结掉 D13/D14）** ★

> **这一节是压缩上下文后的第一读物。** 它写清了：M4-8.5/M4-10 各自落成了什么样、
> **下一步 M4-11 要做什么**、之后三条 API 兼容性分别在哪个阶段、
> 以及一个需要拍板的规划缺口。

**M4-8.5（无饥饿）与 M4-10（SMP 调度）都已完成并板上验证。**
板级自检 **86 passed / 0 failed**，**六组破坏性 A/B 全部检出**。
...
帧搬迁三件事都读每核结构、队列在本核锁里扫）。M4-11 会引入**唯一**的新路径：
`remove_task`（线程退出时把节点从它 `cpu_id` 指的队列里摘掉，
源 OS `scheduler.cpp:562-594`）—— 那时**全局 `scheduler_lock`** 才有第二个用户。

⚠ 换句话说：**M4-10 之前那条"不要再往 `sched_tick` 里加东西"的禁令已经解除**
（它当时防的是"加的东西会被每核化整段重写"），但换成了一条新的：
**退出路径要按源 OS 的 `remove_task` 写，不要自己发明"停着不动"的语义**。
...
| ★ **"挑最短队列"比的是**名册**长度，不是就绪数** ★ | M4-10 实测：启动到相 6 时 CPU0 的名册上躺着十几个**已挂起**的探针（队列是"全部线程的名册"，M4-8.4 对齐源 OS 后的模型），而 CPU1 只有 idle ⇒ **每个新线程都落到 CPU1**，直到它攒到同样多的名册项。源 OS 完全一样（`add_task` 读的就是 `queue->size`）。**不需要改**，但要知道：负载会**成批**倾斜，而不是均分。M4-11 有线程退出之后名册会缩，倾斜自然缓解 |
| **线程栈池只剩几个空槽** | `KSTACK_SLOTS = 32` 而 D14 说线程**没有退出路径** ⇒ 每造一个线程永久占一个槽。M4-10 之后已用掉二十几个，新增自检 `kstack_headroom`（>= 4）在盯。**M4-11 的线程退出才是真正的解**（释放栈），扩池只是权宜 |
...
| **`sched_park_self` 源 OS 没有** | 它是自检探针的便利设施（干完活挂起），靠 `sched_yield` 走同一条切换路径。M4-11 有线程退出后应被"退出"取代 |
...
- **线程池仍只剩几个空槽**：相 6 从 3 个线程加到 4 个（多出的那个是应用级），
  `kstack_headroom`（>= 4）仍然 PASS，但余量已经贴边。
  **M4-11 的线程退出是唯一的解**；在它落地之前，任何"再加一个探针"的改动
  都要先看这个数。
