...
| **M4-10 SMP 调度（D6/D12/D15 一起结案）** | `a158e53` | **81/0**；六组 A/B 全检出；`cpu1 switched=+16 preempted=+15 back=1` |
...
板级自检 **81 passed / 0 failed**，**六组破坏性 A/B 全部检出**。
...
| **M4-10 里的那条时序敏感点** | M4-9.5 起 A/B 窗口内 `invalid` 的时序对代码改动敏感（`0->0` / `0->53`）。M4-10 之后仍是 `0->53`（两次实测一致），敏感点**没定位** ⇒ 与 C 组对照组的实现方式有关，留给 M4-11（那时会加退出路径，正好再看一次）|
...
| **相 4 仍在用 `timer_delay_ms`** | 它是 N 次**串行** 1ms 等待（坑 39），所以"3000ms"实际约 5.4 s —— 只是启动慢，判据不受影响（判据取自相内的快照）。**刻意不改**：那是 M4-8.4 已验证的代码，M4-8.5 只在自己的两处改用 `starve_wait_ms()`。M4-10 重写这一片时一并清 |
