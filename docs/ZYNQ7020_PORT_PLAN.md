...
### 4.5 M4 重新规划：先把调度器与内存管理做实，再上文件系统
...
#### 第三组：设备与文件系统（M4-12 .. M4-16）

| # | 内容 | 依赖 | 说明 |
|---|---|---|---|
| **M4-12** | device manager + `regist_device` + devfs 骨架 | M4-11 | 此时 `mutex` 已可睡眠，不再是退化版 |
| **M4-13** | **VFS 核心 + `tmpfs`** | M4-12 | **第一个完整切片**：建/读/写/列目录全在内存，不需要块设备 |
| **M4-14** | 块设备（SD/arasan）+ `diskio` | 独立，可并行 | 读写扇区 + 写回读 |
| **M4-15** | **FATFS**（先 RAM 盘，后 SD）| M4-13 + M4-14 | RAM 盘与真实块设备走**完全相同**的 `diskio` 路径 |
| **M4-16** | `procfs` / `dev` / `pipe` / `pty` | M4-13 | 逐项 |

#### 明确推迟（不在 M4 范围内）

| 项 | 为什么 |
|---|---|
| `socketfs` / `unixsock` / `dnsfs` / `nmfs` | 网络栈，依赖 lwIP —— M5+ |
| 用户态地址空间 / COW / 按需分页 | M7 |
| `dlinker`（`.sys` 模块）| 另一条独立线索，见 §4.4 的 B5 边界说明 |
| 真正的 CFS 之类的调度策略 | M4 只需要一个正确、可预测的策略（轮转 + 优先级），调优留后 |
...
**建议 M4-15 先接 RAM 盘** —— 与真实块设备走完全相同的 `diskio` 路径，
让 FATFS 的移植**独立于 SD 驱动**先验证完，M4-14 并行推进。

#### 与 B 系列的关系

B5 的难点正是 **`regist_device` 依赖 devfs**，而 devfs 是 M4-12/M4-13 的产物。
M4 做完，B5 才有真实语义可对齐。M4-16 的 `dev` 文件系统同时是
B5"驱动接口兼容旧 XJ380"的**第一个真实用户**。

#### 重新规划后的两个风险（与上一版不同）

**风险 1：在真空里造调度器。**
上一版的风险是"退化实现变成完成态"；这一版换成了相反的：
