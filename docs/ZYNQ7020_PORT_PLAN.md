...
**M0–M4-10 全部完成并板上验证；M4-11.1（串口排他换成真锁）也已完成。
当前板级自检 91 passed / 0 failed。**

**七个**破坏性 A/B 全部成立：搬帧（`a=0 b=0`）、VFP 现场（`bad=(0,7958)`）、
扫描唤醒（`ctl_wakes 0->0`）、无饥饿（`STARVATION: ... missed=4/4`）、
**选核（`pick_cpu0=1` ⇒ `cpu1 switched=+0`）**、不换栈（`b_sp` 落在启动栈）、
**串口锁（`sched_off=+0 ms / alive=+19` vs 旧做法 `+1200 ms / +0`）**。

**下一步 = M4-11.2：线程退出路径（D14 结案）** —— 按 2026-09-13 的决定
**只做源 OS 两段式的第一段（`DEATH` + 让出 + `wfi`），不引入回收器**。
⇒ **先读 `docs/PTASK.md`**（task 子系统专题分册；§4.0 是那次决定的原文，
§4.3 方案 B 是现在要走的路线）。

★ 那次决定的来龙去脉：调研发现**源 OS 的内核线程退出之后不释放内核栈与 TCB**
（`kill_thread` 里那句 `kill_thread0` 是注释掉的，另一条释放路被 `kill_proc` 的
`"Cannot kill System process."` 挡死）。原计划是"我们补一个回收器"，
**后来决定不补** —— 尊重源 OS 的行为，代价是内核栈池**只增不减**
（`KSTACK_SLOTS` 32 × 1 MiB，启动自检已经用到约 30/32），
于是 `kstack_headroom` 从"临时哨兵"升级成**永久容量判据**。
⚠ 但我们的**用法差异**（源 OS 的内核线程是永久基础设施，我们的探针是临时的）
不会因此消失，它变成一条长期约束：任何"每请求一个线程"的用法都得先解决容量。
证据链与"为什么不回收"的完整推演在 `PTASK.md` §2.5/§4.0/§4.1/§9.3。
...
| **★ 决定：不改源 OS 行为（D13 转 yield、D14 不释放）+ M4-11.1 的记录** | 紧随 `25f46ad` 的文档提交 | PTASK §4.0/§4.3/§5/§6；README 的 D13/D14/D17 行 + M4-11.1 一节；坑表 47→50 |
...
### 0.5.7 ★ 续接点：**下一步 = M4-11.2（线程退出路径，D14 结案；★ 不回收 ★）** ★

> **这一节是压缩上下文后的第一读物。** 它写清了：M4-8.5/M4-10/M4-11.1 各自落成了什么样、
> **下一步 M4-11.2 要做什么**、之后三条 API 兼容性分别在哪个阶段、
> 以及一个需要拍板的规划缺口。

**M4-8.5（无饥饿）、M4-10（SMP 调度）、M4-11.1（串口真锁）都已完成并板上验证。**
...
2. ★ **源 OS 的内核线程退出之后不释放资源**：`kill_thread` 只置 `DEATH`
   ...
   代价是 `KSTACK_SLOTS`（32 × 1 MiB）**只增不减**（启动自检已用到约 30/32）
   ⇒ 容量判据从"临时哨兵"升级成**永久判据**。
...
| **11.2** | **线程退出路径（★ 不回收 ★，D14 结案）**：`sched_thread_exit()` = `status = DEATH` → 让出 → `wfi`（源 OS 两段式的**第一段**）；`thread_finish()` 改走它；**`sched_park_self()` 退场**（源 OS 没有这个函数）| 报告项：`kthread_exit_reached`（真有线程走过退出路径）、`kthread_exit_death`（退出后名册里 `status == DEATH`，不是 WAIT）、`kstack_peak_used` / `kstack_headroom`（**永久容量判据**）；破坏性 A/B：退一步用 `sched_park_self()` ⇒ 线程照样不动了，但 `kthread_exit_death` 计数为 0 |
...
| **线程栈池只剩几个空槽（★ 现在是长期约束，不是待修的缺陷 ★）** | `KSTACK_SLOTS = 32`，而按 §4.0 **不回收** ⇒ 每造一个线程**永久**占一个 1 MiB 槽。M4-10 之后已用掉二十几个（`kstack_headroom >= 4` 在盯）。⇒ 处置改为：**容量判据永久化**（11.2 把 `kstack_peak_used` 也报出来）、任何"每请求一个线程"的新用法都必须先解决容量问题。扩池（32→64 ⇒ +32 MiB DDR）随时可选，但那是**容量决定**而不是"补上回收" |
...
- **线程池只剩几个空槽（★ 按 §4.0 已升级为长期容量判据 ★）**：相 6 从 3 个线程加到 4 个
  （多出的那个是应用级），`kstack_headroom`（>= 4）仍然 PASS，但余量已经贴边。
  **按"不回收"的决定，11.2 不会让它回落** ⇒ 11.2 要把 `kstack_peak_used` 也报出来，
  并且**任何"再加一个探针"的改动都要先看这个数**；真要更多线程就是扩池
  （32→64 ⇒ +32 MiB DDR），那是一个**容量决定**，不是"补上回收"。
...
| 11.2 | **线程退出路径（★ 不回收 ★）**：`sched_thread_exit()`（`DEATH` + 让出 + `wfi`，源 OS `process_exit` 的形状）+ `thread_finish()` 改走它 | 板上：`kthread_exit_reached`、`kthread_exit_death`（名册里 `status == DEATH`）、`kstack_peak_used` / `kstack_headroom`（**永久容量判据**）|
| ~~11.3~~ | ~~收尾：`sched_park_self` 退场、结案文档~~ | 并入 11.2：它在那一步变成无调用者的死代码，按 D10/D11 当场删 |
| A/B | 退一步用 `sched_park_self()`（旧的 WAIT 挂起）⇒ 线程照样不动了，但名册里 `status` 是 WAIT 而不是 DEATH ⇒ `kthread_exit_death` 计数为 0 | 与前七组同一风格：**预期它坏**，用来证明"退出语义真的接上了"而不是"线程恰好停住了" |
