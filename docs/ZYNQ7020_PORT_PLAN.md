...
**M0–M4-9.5 全部完成并板上验证；M4-8.4 也已完成；D7 已结案。当前板级自检 70 passed / 0 failed。**

四个破坏性 A/B 全部成立：搬帧（`a=0 b=0`）、VFP 现场（`bad=(7914,0)`）、
扫描唤醒（`ctl_wakes 0->0`）、不换栈（`b_sp` 落在启动栈）。

**下一步 = M4-8.5 无饥饿判据**（用户已确认"现在开始"）。设计已写进 §0.5.7，
**本轮只写计划、尚未动代码**。之后是 M4-10（照源 OS，范围见 §0.5.7）→ M4-11 → M4A 系列。
...
### 0.5.7 ★ 续接点：**下一步 = M4-8.5 无饥饿判据**（已确认，设计已定，尚未动代码）★

> **这一节是压缩上下文后的第一读物。** 它写清了：M4-9/M4-9.5/M4-8.4/D7
> 各自落成了什么样、**下一步 M4-8.5 要怎么做的全部细节**、之后 M4-10 的已核实范围、
> 以及三条 API 兼容性分别在哪个阶段、和一个需要拍板的规划缺口。

**M4-9（抢占）已完成并板上验证。** 这一节从"待办"改成"已完成的样子 +
下一站在哪"，免得下次读到旧文字还以为要重做。

#### 已完成的后四步（摘要；每步的细节在各自的提交说明里）

| 步 | 提交 | 落成了什么样 |
|---|---|---|
| **M4-9 搬帧** | `c45431b` + `79947bc` | 帧路径 + **`svc` 陷阱式让出**（源 OS 的 `scheduler_yield` 就是 `scheduler_ticks=TIME_SLICE; int $32`，不是另一套切换）。`sched_ctx_switchable()` 四条绊线：pc==0 / A+模式域 / sp 4 对齐 / sp 在自己栈区里。破坏性 A/B `g_reloc_skip` ⇒ 两个线程**一次都跑不起来** |
| **M4-9.5 VFP 现场** | `53da560` | 补 M4-6 **写明却从没实现**的账（全树零条 VFP 传输指令）。★ 存/取**卡在 C 调用链的两端**（`_vec_irq` 里 C 之前一条 `bl`、所有 C 返回之后一条 `bl`），因为 GCC 在本内核里已用 `vldr d16/vstr d16` 做 64 位清零 ⇒ 判据里不能含"编译器不会用 VFP"这个前提。地址由 **C 算好放进 `percpu_t`**（汇编只读每核结构的固定偏移，不碰 TCB 布局）。`sizeof(percpu_t)` 56→64 |
| **M4-8.4 周期状态行** | `4e98d1d` | 第一个**会睡会醒**的真实负载。★ 顺带补上 **D4**：队列改 **FIFO**、选取改**全表扫描**（源 OS `select_next_task_safe`）、`sched_queue_scan` 里**带唤醒**、`current` 与睡眠任务都留在队列里 ⇒ `sched_tick` 里那套"一进一出"的队列维护**整段消失**。判据是**唤醒延迟**（"顺序"的可观测量，不是比例）：`wakes=5 late_max=3998us`，有纯占用线程霸着 CPU 时仍 0.1~0.4ms |
| **D7 调用边界对齐** | `d03a639` | `bic sp, sp, #7` **一条指令**；对齐的是**调 C 的边界**，不是帧（帧不能挪 —— 见坑 32）。硬自检 `c_handler_sp_aligned` 必须 0 |
...
#### 下一步：**M4-8.5 无饥饿判据**（用户已确认"现在开始"，本步只写计划、**尚未动代码**）
...
它是 §4.5 那三层验证分工里 **"四条机制判据"的最后一条**。
另外三条：切换正确（M4-7 ✅）、抢占真发生（M4-9 ✅）、idle 真 idle（M4-8 ✅）。

★ **它不是**公平性/比率测试 —— 计划明写"**任何策略在等权 + 纯占用负载下都会通过**"，
「公平份额 ≈ 理论值」「EEVDF 与 round-robin 有别」两条**列为不验**。
要证的是这条**二值**性质：

> **一个可运行的线程，永远不会被无限期地漏掉。**

**为什么现在才做得了**：它是唯一一条"没有抢占就无法观测"的判据 ——
协作式调度下"这个线程没被选中"和"它压根没让出"是同一件事，分不开。
...
| **"可运行线程"怎么定义** | 监视器只盯**它自己造、且从不睡、从不挂起**的线程 | 那些"应当一直在推进"。用 `status ∈ {RUNNING, START, CREATE}` 且非 idle |
| **谁观察**（监视器自己被饿着怎么办）| **用 `sched_sleep_ns` 的睡眠式监视器** | 正是 M4-8.4 刚建好的机制（实测唤醒延迟 0.1~0.4 ms）。这样监视器不参与竞争，**不会因为自己被饿着而漏报** |
| **"推进过 ≥1"取哪个量** | 取**线程自己的计数器**，不是"被选中过" | 它同时覆盖"选中了但还没干活就被换下"这个方向 |

##### 三层判据（按既定分工：策略→宿主，机制→板上）

1. **宿主（策略，穷尽）**：用模拟 tick 时钟反复调 `sched_select_next()`，
   断言 K 个线程里每个都在 N 轮内被选中至少一次。可穷尽扫 K、片长、睡眠模式。
   ★ **这是无饥饿的策略判据。**
2. **板上（机制，二值）**：K=4 个**绝不让出**的线程 + 一个睡眠式监视器，
   判据"每 64 tick 内每个计数都涨过"。
   ⚠ 与 M4-9 的增量在哪要写清楚：M4-9 的 `saw=(1,1)` 已证明 **K=2** 会交错；
   这里的增量是 **K>2**、**avg_vruntime 闸门**、以及**常驻监视**（不是一次性检查）。
3. **破坏性 A/B（证明检出器有区分能力）**：新增 `g_pick_sticky` ——
   `sched_select_next` 在 current 可运行时**总是返回 current**
   （严格非抢占，等价于 M4-8 的行为）⇒ 其余线程必然饿死 ⇒ 监视器必须报警。
   ⚠ 与前四个 A/B 同一风格：**预期它坏**。

##### 诚实说：它现在的价值是"回归监视器"

按现在的代码，**饿死在构造上不会发生**（`sched_select_next` 结尾的
`if (best == NULL) best = fallback;` 保证只要有候选就有人被选中）。
所以 M4-8.5 不是"今天能找到 bug"，而是：

- 守住**下一次**改动（尤其 M4-10 的每核队列 + 锁 —— "任务落在所有队列之外"是那里的高频形态，
  本次已经手撞过一次同类）；
- 补上四条机制判据里的最后一条，让 M4 的验收账完整。

##### 收尾要做的

- 板上自检项 `sched_no_starvation`（二值）+ 宿主 `tests/test_arm32_sched.py` 新增一节
- README §11 加 M4-8.5 一节；**不新增退化条目**（这是新判据，不是退化）
- 提交：实现与文档分两次（本项目惯例）

#### 再之后：**M4-10 照源 OS**（用户已拍板，范围已核实）
...
| **心跳区 32 槽已满** | 再加诊断量就没地方了，而 M4-10 还要**按核**读数。扩容 OCM 布局还是删旧槽，**要拍板** |
...
| **M4-8.5** | **无饥饿判据** | M4-8.4 | ⬜ **下一步**（设计已定死在 §0.5.7，尚未动代码）|
...
| **M4-8.5** | 无饥饿判据:每 N 个 tick 内每个可运行线程都推进过 ≥1 | 板上二值 | ⬜ **下一步**（用户已确认"现在开始"）。⚠ 计划这一行太薄，**四件事已定死在 §0.5.7**：N=64 / 只盯不睡不挂起的线程 / **睡眠式监视器** / 取线程自己的计数。另加宿主穷尽测与 `g_pick_sticky` A/B |
