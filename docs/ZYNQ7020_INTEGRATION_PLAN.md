...
| **D5** | **心跳区扩容** | (a) 扩 OCM 布局 (b) 删旧槽 | ✅ **2026-09-18 选 (a)：32 → 64 槽，已实施（`6cf135d`）**。取证：34 个 `HB_SLOT_*` 名字**每个都有写者**，没有一个死槽可删；而心跳是"串口不可用时唯一还能观测的通道"，且**新诊断量不能改放 DDR**（写回可缓存 ⇒ JTAG 直读物理内存会读到陈旧值）⇒ 扩容是唯一正确选项。代价常数级：两个常量 + **7 行 / 5 个文件的硬编码**（`tmp-test/guard_trip.py:92`、`jtag/inject_fault.tcl:18,21`、`jtag/run_kernel_uart.tcl:206,212`、`arch/arm32/include/arch/fault_test.h:11-19`），有两条 `_Static_assert` 在编译期护栏，**不用动页表**。上板：97/0 + 八组 A/B 全检出；★ 且做了**区分性**验证 —— "启动后读保留区全 0"不承重（`rst -system` 自己就清 OCM），改为"`con` 之前种哨兵、启动后必须被擦掉"（对照 `rst -processor` 哨兵存活），并验证从新地址注入仍能触发 Data Abort |
...
4. ★ **C 与 C++ 的边界**（2026-09-18 立，M4A-1.2a）★：
   **移植侧的 C 文件看不到上游头文件；上游的 C++ 文件看不到移植侧头文件。**
   两边唯一的交界是"C++ 导出 C 链接符号 + 移植侧给出 C 声明"，而那条交界由
   契约测试钉住（`tests/test_arm32_device.py` 的 `id_alloc` / `device_t` 比对）。
   <BR>**为什么必须是硬规矩**：实测上游头文件在 C 里**编不过**，四条证据 ——
   `include/stdint.h:20` 的 `typedef char int8_t`（ARM 上 `char` 默认无符号 ⇒
   上游那条在 ARM 上本身就是错的）、`:38` 的 `#define NULL 0`、
   `include/krlibc.h:22` 的 `typeof(nullptr)`、`:217` 的 `static memmove`。
   构建图上就是 `arm32_cc` 与 `arm32_cxx` 两条规则**各带自己的 `-I`**。
   <BR>⚠ 附带后果（也踩过）：仓库自己的 `include/stdint.h:41` 有
   `#define bool _Bool`，与移植侧的 `typedef u8 bool` 会让同一个 TU 里 `bool`
   变成两种类型 ⇒ 已把移植侧也改成同样的宏形式（`#ifndef` 守卫）。
