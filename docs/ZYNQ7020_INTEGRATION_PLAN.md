...
| 谁来决定下一步？ | 有两件事**要人拍板**：① 用户态该归属哪个阶段（计划 §4.6 只到 M4A-3，而 M5 验收要跑 `shell.elf`）② OCM 心跳区 32 槽已满（扩容还是删旧槽） |
...
3. **用户态与 syscall 层（M4A-4 或 M7，待拍板）**：`syscall_regs.h` + `SVC` 分发 + ELF32 加载 + 用户页表属性；
...
| **M4A-4 用户态与 syscall（★ 待拍板是否新增 ★）** | `kernel/syscall/**`、`kernel/user/**`、`user/xapi/**` | `syscall_regs.h` + `SVC` 分发 + `switch_to_user` + ELF32 加载 + 用户栈/页表属性 | 能跑一个静态链接的 32 位用户程序（最小 syscall：write/exit） |
| **M5 / M6** | VFS/FATFS、网络栈、模块框架 | SD 驱动、GEM 网卡、镜像打包（`bootgen`/SD 启动） | 挂载 `/system`；网络收发；模块加载 |
| **M7** | syscall 处理体（5,313 行）、XAPI、`shell.elf` | 进程层（PCB/进程组/fork/execve）、syscall 号表落地 | **跑起 `shell.elf`** —— 那时"上游代码在 ARM 上真的活了" |

---

## 6. 还没拍板的决策（写在这里，别在实施时才发现）

| # | 决策 | 选项 | 影响面 |
|---|---|---|---|
| **D1** | **syscall 号表策略**（计划 §3 的 R1） | (a) 保留 Linux x86_64 号 + 号翻译层 (b) ARM 独立号表 | 决定用户态是否需要"号翻译"；SXAH 私有号**无论如何都要重编号**（57 位装不进 `r7`），而全部用户态本来就要重建 ⇒ 代价比原估计小 |
| **D2** | **内存分配器用哪一份** | (a) 上游 `kernel/memory/**` + 换页表后端 (b) 保留 ARM 侧自写那份（宿主已穷尽测）+ 对齐接口 | 影响 `palloc/heap/vmap` 的接口形状；两份都无汇编，差别在"谁的测试更硬"与"谁与上游 ABI 更贴" |
| **D3** | **是否支持 `.sys` 模块** | (a) 支持（需 ARM 动态链接器 + 导出面） (b) 暂不支持，驱动先内建 | 影响 M4A-2/M5 的驱动放哪儿；上游的模块 ABI 只导出了约 76 个符号，且**缺** `spin_lock`/`mutex_*`/EOI/中断注册 ⇒ 直接照搬会导致驱动只能轮询（计划 §5 末尾） |
| **D4** | **用户态归属哪个阶段** | (a) 新增 M4A-4（建议） (b) 并进 M7，并把 M5 验收降级 | **这是当前最阻塞的规划缺口**：M5/M6 的验收都要用户态先存在（计划 §0.5.7 末尾） |
| **D5** | **心跳区扩容** | (a) 扩 OCM 布局 (b) 删旧槽 | 决定后续诊断量走哪里（现在 32 槽已满，新的读数走串口/自检报告） |
| **D6** | **与上游的同步方式** | (a) 持续 rebase 到上游 `main` (b) 到某个里程碑再合 | 因为**上游文件几乎没改**（唯一共享改动点是 `tools/gen_ninja.py`，+261/−4），冲突面很小；将来可把 ARM 构建图拆成 `tools/gen_ninja_arm.py` 让这个面变成零 |
